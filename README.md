...
| `MES500TSwingMapV3.cs` | **Aktuální SwingMap** — jako V2 + **% pravděpodobnost** u PEAK? (síla signálu vrcholu) |
...
3. Na graf **MES** (500 tick) přidej indikátor:
   - swing mapa: **MES500TSwingMapV3** (doporučeno) nebo V2 / V1
...
| Verze | Kdy použít |
|-------|------------|
| **V3** | Ruční obchod — PEAK? s % + ◆ PEAK po pullbacku (nejlepší filtr falešných peaků) |
| **V2** | Stejné jako V3, ale bez % u PEAK? |
| **V1** | Základ — jeden PEAK signál bez dvoustupňového potvrzení |

### Klíčové parametry (V2/V3, skupina 4. PEAK)
...
- **MES500TSwingMapV3** — swing mapa s VSTUP/PEAK?/◆ PEAK a % u varování (doporučeno pro ruční exit)
