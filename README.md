...
| `MES500TSwingMapV4.cs` | **Aktuální SwingMap** — V3 + **statistiky** (win rate, ticky, USD za VSTUP→◆ PEAK) |
| `MES500TSwingMapV4Light.cs` | V4 Light — statistiky jen za **poslední 1h**, bez swing čar a strength panelu |
...
3. Na graf **MES** (500 tick) přidej indikátor:
   - swing mapa: **MES500TSwingMapV4** (doporučeno) nebo V4Light / V3 / V2 / V1
...
| Verze | Kdy použít |
|-------|------------|
| **V4** | V3 + panel statistik (win rate, ticky, USD) pro páry VSTUP→◆ PEAK |
| **V4 Light** | jako V4, statistiky za 1h, méně kreslení (slabší PC) |
| **V3** | PEAK? s % + ◆ PEAK po pullbacku |
| **V2** | Stejné jako V3, ale bez % u PEAK? |
| **V1** | Základ — jeden PEAK signál bez dvoustupňového potvrzení |

### Statistiky (V4 / V4 Light, skupina 5. Statistiky)
...
| Parametr | V4 | V4 Light |
|----------|-----|----------|
| Stats Lookback Hours | 0 (celá historie) | 1 |
| Max Stats Trades | 500 | 80 |
| Show Swing Lines | ON | OFF |
| Show Strength Panel | ON | OFF |

### Klíčové parametry (V2/V3/V4, skupina 4. PEAK)
...
- **MES500TSwingMapV4** — swing mapa + statistiky win rate / USD (doporučeno)
- **MES500TSwingMapV4Light** — V4 pro slabší PC (statistiky za 1h)
