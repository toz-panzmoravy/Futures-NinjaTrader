...
{
	public class MES500TSqueezeMomentumV39Light : Indicator
	{
		...
		[NinjaScriptProperty]
		[Display(Name = "Show Trend Leg Entries", Description = "V39: NÁKUP/PRODEJ uprostřed trendu (KC mid + momentum), ne jen u pásma.", Order = 1, GroupName = "22. V39 Manual Entries")]
		public bool ShowTrendLegEntries { get; set; }

		[NinjaScriptProperty]
		[Display(Name = "Relax Re-Entry In Trend", Description = "V39: Po ztrátě povolit nový vstup ve směru KC trendu.", Order = 2, GroupName = "22. V39 Manual Entries")]
		public bool RelaxReEntryInTrend { get; set; }

		[NinjaScriptProperty]
		[Display(Name = "Show Blocked Entry Hints", Description = "V39: Ukázat NÁKUP/PRODEJ i když simulace ještě neotevře (filtr re-entry).", Order = 3, GroupName = "22. V39 Manual Entries")]
		public bool ShowBlockedEntryHints { get; set; }

		[NinjaScriptProperty]
		[Range(20, 200)]
		[Display(Name = "Max Marker History Bars", Description = "Light: starší značky na grafu se mažou (výchozí 60 svíček).", Order = 1, GroupName = "23. V39 Light")]
		...
		[NinjaScriptProperty]
		[Range(1, 15)]
		[Display(Name = "Popup Duration Sec", Description = "Kolik sekund popup zůstane na obrazovce (výchozí 2 s).", Order = 2, GroupName = "17. Windows Popup")]
		public int PopupDurationSec { get; set; }

		[NinjaScriptProperty]
		[Display(Name = "Popup On Approach", Description = "Popup u hintů NÁKUP / PRODEJ / ZAVŘÍT.", Order = 3, GroupName = "17. Windows Popup")]
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500T V39 Light — pro slabší PC. Stejná logika jako V39 (trend leg + band reversal + exit assessment), méně kreslení, značky max 60 svíček zpět.";
				Name = "MES500TSqueezeMomentumV39Light";
				...
				ShowTrendLegEntries = true;
				RelaxReEntryInTrend = true;
				ShowBlockedEntryHints = true;
				...
				ShowTradeLines = true;
				ShowReleaseMarker = true;
				ShowStatsPanel = false;
				ShowLiveSituationPanel = false;
				ShowKcMid = true;
				ShowEntryTrigger = true;
		...
		protected override void OnBarUpdate()
		{
			...
			if (tradeState == TradeState.Flat && IsEntryCooldownElapsed())
			{
				...
				else if (CanEvaluateEntry())
				{
					...
					else if (ManualTradeMode && ShowBlockedEntryHints)
						TryDrawBlockedEntryHint(longSignal, shortSignal, longTrigger, shortTrigger, hist0, hist1, hist2);
				}
			}
			...
			if (tradeState == TradeState.Flat)
			{
				...
				if (longScore >= longMinScore && longScore > shortScore
					...
					&& IsLongEntryImminent(
						squeezeState, releaseDirection, longEntryBlocked, kcMid, kcUpper, kcLower, entryBuffer,
						hist0, hist1, hist2, hist3, macdTangle))
					DrawApproachHint(ApproachKind.Buy, BuildApproachBandSubtext(true));
				else if (shortScore >= shortMinScore && shortScore > longScore
					&& PassesReEntryApproachFilter(false)
					&& IsShortEntryImminent(
						squeezeState, releaseDirection, shortEntryBlocked, kcMid, kcUpper, kcLower, entryBuffer,
						hist0, hist1, hist2, hist3, macdTangle))
					DrawApproachHint(ApproachKind.Sell, BuildApproachBandSubtext(false));
			}
			...
			bool macdTangle)
		{
			...
			if (EnableBandReversalEntries && IsNearUpperBand(kcUpper, entryBuffer))
				return 0;
			...
			bool macdTangle)
		{
			...
			if (EnableBandReversalEntries && IsNearLowerBand(kcLower, entryBuffer))
				return 0;
			...
			bool macdTangle)
		{
			...
			string tag = "MES500TV39L_Ring_" + CurrentBar;
		...
		private bool PassesReEntryFilter(bool isLong, EntryTrigger trigger, SignalGrade grade)
		{
			...
			{
				if (ManualTradeMode && RelaxReEntryInTrend
					&& (trigger == EntryTrigger.Continuation || trigger == EntryTrigger.Reversal)
					&& HasKcMidSlope(isLong, Math.Max(2, EntryMinKcSlopeBars)))
				{
					if (ReEntryRequireGradeBOrBetter && grade == SignalGrade.C)
						return false;
					return CurrentBar - lastCloseBar >= Math.Max(2, ReEntryMinBarsSinceClose / 2);
				}
				return false;
			}
		...
		{
			string prefix = "MES500TV39L_Hint_" + bar;
		...
		{
			string tag = "MES500TV39L_" + tradeId + "_ExitWarn";
		...
		{
			string tag = "MES500TV39L_Hint_" + CurrentBar;
			...
			if (extraLine != null && extraLine.Length > 0)
			{
				bool appendSubtext = kind == ApproachKind.Close ? ShowExitAssessment : true;
				if (appendSubtext)
				{
					if (label.Length > 0)
						label += "\n" + extraLine;
			...
			if (PopupOnApproach)
				TryShowApproachPopup(kind, BuildApproachPopupMessage(kind, extraLine));
		...
		private void ResetTradeTracking()
		{
			...
			postPeakMaxTicks = 0;
			openIsReEntry = false;
			RemoveDrawObject("MES500TV39L_Trail");
			RemoveDrawObject("MES500TV39L_Char");
			RemoveDrawObject("MES500TV39L_Char_Sub");
			RemoveDrawObject("MES500TV39L_Char_Hint");
		}
		...
		private bool IsShortTrendLegSetup(double kcMid, double hist0, double hist1, double hist2, double hist3)
		{
			if (Close[0] >= kcMid || Close[0] >= Open[0])
				return false;
			if (hist0 >= 0)
				return false;
			if (EntryMinKcSlopeBars > 0 && !HasKcMidSlope(false, EntryMinKcSlopeBars))
				return false;
			if (IsShortExhaustion(hist0, hist1, hist2, hist3))
				return false;
			return hist0 < hist1 || HasMinHistStep(hist0, hist1);
		}

		private bool IsLongTrendLegSetup(double kcMid, double hist0, double hist1, double hist2, double hist3)
		{
			if (Close[0] <= kcMid || Close[0] <= Open[0])
				return false;
			if (hist0 <= 0)
				return false;
			if (EntryMinKcSlopeBars > 0 && !HasKcMidSlope(true, EntryMinKcSlopeBars))
				return false;
			if (IsLongExhaustion(hist0, hist1, hist2, hist3))
				return false;
			return hist0 > hist1 || HasMinHistStep(hist0, hist1);
			...
			out EntryTrigger trigger)
		{
			...
			if (ShowTrendLegEntries && !entryBlocked && !macdTangle
				&& IsLongTrendLegSetup(kcMid, hist0, hist1, hist2, hist3))
			{
			...
			out EntryTrigger trigger)
		{
			...
			if (ShowTrendLegEntries && !entryBlocked && !macdTangle
				&& IsShortTrendLegSetup(kcMid, hist0, hist1, hist2, hist3))
			{
				trigger = EntryTrigger.Continuation;
		...
		private string BuildApproachBandSubtext(bool isLong)
		{
			return isLong
				? "Odraz od dolního pásma — brzy NÁKUP"
				: "Odraz od horního pásma — brzy PRODEJ";
		}

		private string BuildApproachPopupMessage(ApproachKind kind, string extraLine)
		{
			if (extraLine != null && extraLine.Length > 0)
				return extraLine;
			return GetApproachHint(kind);
		}

		private string BuildEntryAlertText(bool isLong, int tradeId, SignalGrade grade, EntryTrigger trigger)
		{
			string dir = isLong ? "NÁKUP" : "PRODEJ";
			string triggerText = GetEntryTriggerText(trigger);
			if (triggerText.Length > 0)
				return dir + " #" + tradeId + " · " + GetSignalGradeText(grade) + " · " + triggerText;
			return dir + " #" + tradeId + " · " + GetSignalGradeText(grade);
		}

		private string BuildExitWarningAlertText(int tradeId, string pnlText, ExitAssessment assessment, CloseReason reason, bool wasLong)
		{
			string detail = ShowExitAssessment
				? FormatExitAssessmentHint(assessment, reason, wasLong)
				: GetCloseReasonHint(reason, wasLong);
			return "BRZY ZAVŘÍT #" + tradeId + " " + pnlText + " · " + detail;
		}

		private string BuildCloseAlertText(int tradeId, string pnlText, CloseReason reason, bool wasLong, ExitAssessment assessment, CloseHoldVerdict holdVerdict)
		{
			string alertText = "ZAVŘÍT #" + tradeId + " " + pnlText;
			if (ShowCloseReason)
		...
		private string GetApproachHint(ApproachKind kind)
		{
			switch (kind)
			{
				case ApproachKind.Buy: return "Blíží se signál NÁKUP";
				case ApproachKind.ReBuy: return "Blíží se ZPĚTNÝ NÁKUP";
				case ApproachKind.Sell: return "Blíží se signál PRODEJ";
				case ApproachKind.ReSell: return "Blíží se ZPĚTNÝ PRODEJ";
				default: return "Blíží se signál ZAVŘÍT";
			}
		}
		...
		private void UpdateLiveTradeCharacter(bool isLong, double kcMid, double hist0, double hist1, double hist2)
		{
			...
			Draw.Text(this, "MES500TV39L_Char", false, mainLabel, 0, labelY, 0, brush,
		...
		private void DrawActiveTrailLine(bool isLong)
		{
			...
			Draw.Line(this, "MES500TV39L_Trail", false, 1, trailStopPrice, 0, trailStopPrice,
		...
		private void UpdateRecoveryWatch()
		{
			...
			{
				recoveryWatchActive = false;
				RemoveDrawObject("MES500TV39L_Missed");
				return;
			}
			...
			if (favorable >= RunnerTargetTicks && !recoveryMissShown)
			{
				...
				Draw.TextFixed(this, "MES500TV39L_Missed", text, TextPosition.BottomLeft,
		...
		private void PruneHistoricalMarkers()
		{
			...
			{
				string hintTag = "MES500TV39L_Hint_" + bar;
				RemoveDrawObject(hintTag + "_Sq");
				RemoveDrawObject(hintTag + "_Txt");
				RemoveDrawObject(hintTag + "_Hint");
				RemoveDrawObject("MES500TV39L_Ring_" + bar);

				string entryHintLong = "MES500TV39L_EntryHint_" + bar + "_L";
				RemoveDrawObject(entryHintLong + "_Arrow");
				RemoveDrawObject(entryHintLong + "_Text");
				string entryHintShort = "MES500TV39L_EntryHint_" + bar + "_S";
				RemoveDrawObject(entryHintShort + "_Arrow");
				RemoveDrawObject(entryHintShort + "_Text");

				string releaseTag = "MES500TV39L_Release_" + bar;
		...
		{
			RemoveDrawObject("MES500TV39L_Situation");
		...
		private void DrawExitWarning(int tradeId, bool wasLong, CloseReason reason)
		{
			...
			if (EnableExitWarningAlerts && reason != lastExitWarningReason)
			{
				...
				string alertDetail = ShowExitAssessment
					? FormatExitAssessmentHint(assessment, reason, wasLong)
					: GetCloseReasonHint(reason, wasLong);
				Alert(tag, Priority.High, BuildExitWarningAlertText(tradeId, pnlText, assessment, reason, wasLong),
					"Alert4.wav", 10, Brushes.Transparent, Brushes.Black);
				if (EnableTaskbarFlash && FlashOnClose)
					FlashTaskbar();
				if (PopupOnClose)
					TryShowScreenPopup("ExitWarn_" + tradeId + "_" + reason, "BRZY ZAVŘÍT #" + tradeId, pnlText + " · " + alertDetail, Colors.Gold);
			}
		...
		private void DrawReleaseMarker(SqueezeReleaseDirection direction)
		{
			...
			string tag = "MES500TV39L_Release_" + CurrentBar;
		...
		private void TryDrawBlockedEntryHint(
			bool longSignal,
			bool shortSignal,
			EntryTrigger longTrigger,
			EntryTrigger shortTrigger,
			...
			double hist2)
		{
			if (!ShowSignals && !ShowSignalLabels)
				return;

			if (longSignal && !shortSignal)
			{
				SignalGrade grade = GetSignalGrade(true, longTrigger, hist0, hist1, hist2);
				DrawBlockedEntryHint(true, longTrigger, grade);
			}
			else if (shortSignal && !longSignal)
			{
				SignalGrade grade = GetSignalGrade(false, shortTrigger, hist0, hist1, hist2);
				DrawBlockedEntryHint(false, shortTrigger, grade);
			}
		}

		private void DrawBlockedEntryHint(bool isLong, EntryTrigger trigger, SignalGrade grade)
		{
			string tag = "MES500TV39L_EntryHint_" + CurrentBar + (isLong ? "_L" : "_S");
			string label = isLong ? "NÁKUP" : "PRODEJ";
			string subLabel = GetEntryTriggerText(trigger) + " · " + GetSignalGradeText(grade);
			string fullLabel = label + "\n" + subLabel;
			string hint = BuildEntryHint(isLong, trigger, grade);
			Brush brush = isLong ? LongSignalBrush : ShortSignalBrush;
			...
			if (isLong)
			{
				...
				if (ShowSignals)
					Draw.ArrowUp(this, tag + "_Arrow", false, 0, arrowY, brush);
				if (ShowSignalLabels)
					Draw.Text(this, tag + "_Text", false, fullLabel, 0, labelY, 0, brush,
						signalFont, TextAlignment.Center, Brushes.Black, Brushes.White, 100);
			}
			else
			{
				...
				if (ShowSignals)
					Draw.ArrowDown(this, tag + "_Arrow", false, 0, arrowY, brush);
				if (ShowSignalLabels)
					Draw.Text(this, tag + "_Text", false, fullLabel, 0, labelY, 0, brush,
						signalFont, TextAlignment.Center, Brushes.Black, Brushes.White, 100);
			}

			if (EnableAlerts && FlashOnEntry)
				FlashTaskbar();
			if (PopupOnEntry)
		...
		{
			string tag = "MES500TV39L_" + tradeId + "_Long";
		...
		{
			string tag = "MES500TV39L_" + tradeId + "_Short";
		...
		private void DrawCloseSignal(int tradeId, bool wasLong, CloseReason reason, int entryBar, double entryPrice, double pnlTicks, CloseHoldVerdict holdVerdict)
		{
			...
			string tag = "MES500TV39L_" + tradeId + "_Close";
			...
			{
				Alert(tag, Priority.Medium, BuildCloseAlertText(tradeId, pnlText, reason, wasLong, assessment, holdVerdict), "Alert3.wav", 10, Brushes.Transparent, Brushes.Black);
				if (FlashOnClose)
					FlashTaskbar();
			}
		...
		private void DrawStatsPanel()
		{
			...
			Draw.TextFixed(this, "MES500TV39L_Stats", statsText, TextPosition.TopRight,
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSqueezeMomentumV39Light[] cacheMES500TSqueezeMomentumV39Light;
		public MES500TSqueezeMomentumV39Light MES500TSqueezeMomentumV39Light(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, int maxMarkerHistoryBars, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return MES500TSqueezeMomentumV39Light(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, maxMarkerHistoryBars, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public MES500TSqueezeMomentumV39Light MES500TSqueezeMomentumV39Light(ISeries<double> input, bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, int maxMarkerHistoryBars, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			if (cacheMES500TSqueezeMomentumV39Light != null)
				for (int idx = 0; idx < cacheMES500TSqueezeMomentumV39Light.Length; idx++)
					if (cacheMES500TSqueezeMomentumV39Light[idx] != null && cacheMES500TSqueezeMomentumV39Light[idx].ManualTradeMode == manualTradeMode && cacheMES500TSqueezeMomentumV39Light[idx].MinBarsBeforeSoftClose == minBarsBeforeSoftClose && cacheMES500TSqueezeMomentumV39Light[idx].AllowEarlyFlipClose == allowEarlyFlipClose && cacheMES500TSqueezeMomentumV39Light[idx].ShowHoldWindowTag == showHoldWindowTag && cacheMES500TSqueezeMomentumV39Light[idx].SuppressExitWarningInHold == suppressExitWarningInHold && cacheMES500TSqueezeMomentumV39Light[idx].EnableTrendRunnerMode == enableTrendRunnerMode && cacheMES500TSqueezeMomentumV39Light[idx].TrendRunnerMinPeakTicks == trendRunnerMinPeakTicks && cacheMES500TSqueezeMomentumV39Light[idx].TrendRunnerKcSlopeBars == trendRunnerKcSlopeBars && cacheMES500TSqueezeMomentumV39Light[idx].TrendRunnerFadeBars == trendRunnerFadeBars && cacheMES500TSqueezeMomentumV39Light[idx].TrendRunnerGivebackMult == trendRunnerGivebackMult && cacheMES500TSqueezeMomentumV39Light[idx].TrendRunnerTrailMult == trendRunnerTrailMult && cacheMES500TSqueezeMomentumV39Light[idx].EntryMinKcSlopeBars == entryMinKcSlopeBars && cacheMES500TSqueezeMomentumV39Light[idx].BlockCounterTrendIgnition == blockCounterTrendIgnition && cacheMES500TSqueezeMomentumV39Light[idx].SuppressExhaustionInRunner == suppressExhaustionInRunner && cacheMES500TSqueezeMomentumV39Light[idx].OppositeSignalNeedsFade == oppositeSignalNeedsFade && cacheMES500TSqueezeMomentumV39Light[idx].ShowExitAssessment == showExitAssessment && cacheMES500TSqueezeMomentumV39Light[idx].EnableBandReversalEntries == enableBandReversalEntries && cacheMES500TSqueezeMomentumV39Light[idx].BandReversalNearTicks == bandReversalNearTicks && cacheMES500TSqueezeMomentumV39Light[idx].AllowReversalInSqueeze == allowReversalInSqueeze && cacheMES500TSqueezeMomentumV39Light[idx].ShowTrendLegEntries == showTrendLegEntries && cacheMES500TSqueezeMomentumV39Light[idx].RelaxReEntryInTrend == relaxReEntryInTrend && cacheMES500TSqueezeMomentumV39Light[idx].ShowBlockedEntryHints == showBlockedEntryHints && cacheMES500TSqueezeMomentumV39Light[idx].MaxMarkerHistoryBars == maxMarkerHistoryBars && cacheMES500TSqueezeMomentumV39Light[idx].ShowApproachRing == showApproachRing && cacheMES500TSqueezeMomentumV39Light[idx].ApproachRingMinScore == approachRingMinScore && cacheMES500TSqueezeMomentumV39Light[idx].ApproachRingSizeTicks == approachRingSizeTicks && cacheMES500TSqueezeMomentumV39Light[idx].ApproachRingMinOpacity == approachRingMinOpacity && cacheMES500TSqueezeMomentumV39Light[idx].ApproachRingMaxOpacity == approachRingMaxOpacity && cacheMES500TSqueezeMomentumV39Light[idx].EnableScreenPopup == enableScreenPopup && cacheMES500TSqueezeMomentumV39Light[idx].PopupDurationSec == popupDurationSec && cacheMES500TSqueezeMomentumV39Light[idx].PopupOnApproach == popupOnApproach && cacheMES500TSqueezeMomentumV39Light[idx].PopupOnEntry == popupOnEntry && cacheMES500TSqueezeMomentumV39Light[idx].PopupOnClose == popupOnClose && cacheMES500TSqueezeMomentumV39Light[idx].ShowCloseHoldAdvice == showCloseHoldAdvice && cacheMES500TSqueezeMomentumV39Light[idx].EnableSignalAutoCorrect == enableSignalAutoCorrect && cacheMES500TSqueezeMomentumV39Light[idx].SignalReviewBars == signalReviewBars && cacheMES500TSqueezeMomentumV39Light[idx].SignalMinBarsBeforeReview == signalMinBarsBeforeReview && cacheMES500TSqueezeMomentumV39Light[idx].FalseSignalAdverseTicks == falseSignalAdverseTicks && cacheMES500TSqueezeMomentumV39Light[idx].SignalConfirmTicks == signalConfirmTicks && cacheMES500TSqueezeMomentumV39Light[idx].BlockWeakReEntrySignals == blockWeakReEntrySignals && cacheMES500TSqueezeMomentumV39Light[idx].ReEntryMinBarsSinceClose == reEntryMinBarsSinceClose && cacheMES500TSqueezeMomentumV39Light[idx].ReEntryRequireGradeBOrBetter == reEntryRequireGradeBOrBetter && cacheMES500TSqueezeMomentumV39Light[idx].BlockReEntryAfterChopClose == blockReEntryAfterChopClose && cacheMES500TSqueezeMomentumV39Light[idx].BlockReEntryAfterLossClose == blockReEntryAfterLossClose && cacheMES500TSqueezeMomentumV39Light[idx].ReEntryApproachMinScore == reEntryApproachMinScore && cacheMES500TSqueezeMomentumV39Light[idx].CntMinKcMidSlopeBars == cntMinKcMidSlopeBars && cacheMES500TSqueezeMomentumV39Light[idx].ShowTrailLine == showTrailLine && cacheMES500TSqueezeMomentumV39Light[idx].ShowSignalGrade == showSignalGrade && cacheMES500TSqueezeMomentumV39Light[idx].ShowTradeCharacter == showTradeCharacter && cacheMES500TSqueezeMomentumV39Light[idx].ChopQuickPeakTicks == chopQuickPeakTicks && cacheMES500TSqueezeMomentumV39Light[idx].ChopGivebackTicks == chopGivebackTicks && cacheMES500TSqueezeMomentumV39Light[idx].RunnerTargetTicks == runnerTargetTicks && cacheMES500TSqueezeMomentumV39Light[idx].RecoveryWindowSec == recoveryWindowSec && cacheMES500TSqueezeMomentumV39Light[idx].EnableTaskbarFlash == enableTaskbarFlash && cacheMES500TSqueezeMomentumV39Light[idx].FlashOnEntry == flashOnEntry && cacheMES500TSqueezeMomentumV39Light[idx].FlashOnClose == flashOnClose && cacheMES500TSqueezeMomentumV39Light[idx].FlashOnApproach == flashOnApproach && cacheMES500TSqueezeMomentumV39Light[idx].ShowApproachHints == showApproachHints && cacheMES500TSqueezeMomentumV39Light[idx].ShowApproachLabels == showApproachLabels && cacheMES500TSqueezeMomentumV39Light[idx].ApproachMinScore == approachMinScore && cacheMES500TSqueezeMomentumV39Light[idx].ApproachNearTicks == approachNearTicks && cacheMES500TSqueezeMomentumV39Light[idx].ApproachOffsetTicks == approachOffsetTicks && cacheMES500TSqueezeMomentumV39Light[idx].ReEntryLookbackBars == reEntryLookbackBars && cacheMES500TSqueezeMomentumV39Light[idx].BbPeriod == bbPeriod && cacheMES500TSqueezeMomentumV39Light[idx].BbStdDev == bbStdDev && cacheMES500TSqueezeMomentumV39Light[idx].KcPeriod == kcPeriod && cacheMES500TSqueezeMomentumV39Light[idx].KcMultiplier == kcMultiplier && cacheMES500TSqueezeMomentumV39Light[idx].MacdFast == macdFast && cacheMES500TSqueezeMomentumV39Light[idx].MacdSlow == macdSlow && cacheMES500TSqueezeMomentumV39Light[idx].MacdSignal == macdSignal && cacheMES500TSqueezeMomentumV39Light[idx].TangleSeparationTicks == tangleSeparationTicks && cacheMES500TSqueezeMomentumV39Light[idx].TangleSlopeTicks == tangleSlopeTicks && cacheMES500TSqueezeMomentumV39Light[idx].RequireThreeBarMomentum == requireThreeBarMomentum && cacheMES500TSqueezeMomentumV39Light[idx].RequireDirectionalRelease == requireDirectionalRelease && cacheMES500TSqueezeMomentumV39Light[idx].EntryBufferTicks == entryBufferTicks && cacheMES500TSqueezeMomentumV39Light[idx].AllowKcMidEntry == allowKcMidEntry && cacheMES500TSqueezeMomentumV39Light[idx].AllowMomentumIgnition == allowMomentumIgnition && cacheMES500TSqueezeMomentumV39Light[idx].AllowMacdCrossEntry == allowMacdCrossEntry && cacheMES500TSqueezeMomentumV39Light[idx].AllowBarBreakEntry == allowBarBreakEntry && cacheMES500TSqueezeMomentumV39Light[idx].AllowTrendContinuationEntry == allowTrendContinuationEntry && cacheMES500TSqueezeMomentumV39Light[idx].AllowPartialSqueezeEntry == allowPartialSqueezeEntry && cacheMES500TSqueezeMomentumV39Light[idx].BlockFullSqueezeOnly == blockFullSqueezeOnly && cacheMES500TSqueezeMomentumV39Light[idx].UseIntrabarEntries == useIntrabarEntries && cacheMES500TSqueezeMomentumV39Light[idx].RequireBounceForContinuation == requireBounceForContinuation && cacheMES500TSqueezeMomentumV39Light[idx].RequireKcMidTrend == requireKcMidTrend && cacheMES500TSqueezeMomentumV39Light[idx].MinHistStepTicks == minHistStepTicks && cacheMES500TSqueezeMomentumV39Light[idx].EntryCooldownBars == entryCooldownBars && cacheMES500TSqueezeMomentumV39Light[idx].KcBreakConfirmBars == kcBreakConfirmBars && cacheMES500TSqueezeMomentumV39Light[idx].MinBarsInTrade == minBarsInTrade && cacheMES500TSqueezeMomentumV39Light[idx].RequireMomentumFadeForClose == requireMomentumFadeForClose && cacheMES500TSqueezeMomentumV39Light[idx].UseExhaustionForClose == useExhaustionForClose && cacheMES500TSqueezeMomentumV39Light[idx].UseAtrTrail == useAtrTrail && cacheMES500TSqueezeMomentumV39Light[idx].AtrTrailMultiplier == atrTrailMultiplier && cacheMES500TSqueezeMomentumV39Light[idx].UseIntrabarExits == useIntrabarExits && cacheMES500TSqueezeMomentumV39Light[idx].ShowExitWarning == showExitWarning && cacheMES500TSqueezeMomentumV39Light[idx].EnableExitWarningAlerts == enableExitWarningAlerts && cacheMES500TSqueezeMomentumV39Light[idx].UseProfitGivebackExit == useProfitGivebackExit && cacheMES500TSqueezeMomentumV39Light[idx].MinProfitForGiveback == minProfitForGiveback && cacheMES500TSqueezeMomentumV39Light[idx].ProfitGivebackTicks == profitGivebackTicks && cacheMES500TSqueezeMomentumV39Light[idx].EnableAlerts == enableAlerts && cacheMES500TSqueezeMomentumV39Light[idx].ShowSignals == showSignals && cacheMES500TSqueezeMomentumV39Light[idx].ShowSignalLabels == showSignalLabels && cacheMES500TSqueezeMomentumV39Light[idx].ShowCloseSignals == showCloseSignals && cacheMES500TSqueezeMomentumV39Light[idx].ShowCloseLabels == showCloseLabels && cacheMES500TSqueezeMomentumV39Light[idx].ShowCloseReason == showCloseReason && cacheMES500TSqueezeMomentumV39Light[idx].ShowTradeLines == showTradeLines && cacheMES500TSqueezeMomentumV39Light[idx].ShowReleaseMarker == showReleaseMarker && cacheMES500TSqueezeMomentumV39Light[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSqueezeMomentumV39Light[idx].ShowLiveSituationPanel == showLiveSituationPanel && cacheMES500TSqueezeMomentumV39Light[idx].ShowKcMid == showKcMid && cacheMES500TSqueezeMomentumV39Light[idx].EnableCloseAlerts == enableCloseAlerts && cacheMES500TSqueezeMomentumV39Light[idx].ShowEntryTrigger == showEntryTrigger && cacheMES500TSqueezeMomentumV39Light[idx].ShowSqueezeBackground == showSqueezeBackground && cacheMES500TSqueezeMomentumV39Light[idx].ArrowOffsetTicks == arrowOffsetTicks && cacheMES500TSqueezeMomentumV39Light[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSqueezeMomentumV39Light[idx].CloseOffsetTicks == closeOffsetTicks && cacheMES500TSqueezeMomentumV39Light[idx].EqualsInput(input))
						return cacheMES500TSqueezeMomentumV39Light[idx];
			return CacheIndicator<MES500TSqueezeMomentumV39Light>(new MES500TSqueezeMomentumV39Light(){ ManualTradeMode = manualTradeMode, MinBarsBeforeSoftClose = minBarsBeforeSoftClose, AllowEarlyFlipClose = allowEarlyFlipClose, ShowHoldWindowTag = showHoldWindowTag, SuppressExitWarningInHold = suppressExitWarningInHold, EnableTrendRunnerMode = enableTrendRunnerMode, TrendRunnerMinPeakTicks = trendRunnerMinPeakTicks, TrendRunnerKcSlopeBars = trendRunnerKcSlopeBars, TrendRunnerFadeBars = trendRunnerFadeBars, TrendRunnerGivebackMult = trendRunnerGivebackMult, TrendRunnerTrailMult = trendRunnerTrailMult, EntryMinKcSlopeBars = entryMinKcSlopeBars, BlockCounterTrendIgnition = blockCounterTrendIgnition, SuppressExhaustionInRunner = suppressExhaustionInRunner, OppositeSignalNeedsFade = oppositeSignalNeedsFade, ShowExitAssessment = showExitAssessment, EnableBandReversalEntries = enableBandReversalEntries, BandReversalNearTicks = bandReversalNearTicks, AllowReversalInSqueeze = allowReversalInSqueeze, ShowTrendLegEntries = showTrendLegEntries, RelaxReEntryInTrend = relaxReEntryInTrend, ShowBlockedEntryHints = showBlockedEntryHints, MaxMarkerHistoryBars = maxMarkerHistoryBars, ShowApproachRing = showApproachRing, ApproachRingMinScore = approachRingMinScore, ApproachRingSizeTicks = approachRingSizeTicks, ApproachRingMinOpacity = approachRingMinOpacity, ApproachRingMaxOpacity = approachRingMaxOpacity, EnableScreenPopup = enableScreenPopup, PopupDurationSec = popupDurationSec, PopupOnApproach = popupOnApproach, PopupOnEntry = popupOnEntry, PopupOnClose = popupOnClose, ShowCloseHoldAdvice = showCloseHoldAdvice, EnableSignalAutoCorrect = enableSignalAutoCorrect, SignalReviewBars = signalReviewBars, SignalMinBarsBeforeReview = signalMinBarsBeforeReview, FalseSignalAdverseTicks = falseSignalAdverseTicks, SignalConfirmTicks = signalConfirmTicks, BlockWeakReEntrySignals = blockWeakReEntrySignals, ReEntryMinBarsSinceClose = reEntryMinBarsSinceClose, ReEntryRequireGradeBOrBetter = reEntryRequireGradeBOrBetter, BlockReEntryAfterChopClose = blockReEntryAfterChopClose, BlockReEntryAfterLossClose = blockReEntryAfterLossClose, ReEntryApproachMinScore = reEntryApproachMinScore, CntMinKcMidSlopeBars = cntMinKcMidSlopeBars, ShowTrailLine = showTrailLine, ShowSignalGrade = showSignalGrade, ShowTradeCharacter = showTradeCharacter, ChopQuickPeakTicks = chopQuickPeakTicks, ChopGivebackTicks = chopGivebackTicks, RunnerTargetTicks = runnerTargetTicks, RecoveryWindowSec = recoveryWindowSec, EnableTaskbarFlash = enableTaskbarFlash, FlashOnEntry = flashOnEntry, FlashOnClose = flashOnClose, FlashOnApproach = flashOnApproach, ShowApproachHints = showApproachHints, ShowApproachLabels = showApproachLabels, ApproachMinScore = approachMinScore, ApproachNearTicks = approachNearTicks, ApproachOffsetTicks = approachOffsetTicks, ReEntryLookbackBars = reEntryLookbackBars, BbPeriod = bbPeriod, BbStdDev = bbStdDev, KcPeriod = kcPeriod, KcMultiplier = kcMultiplier, MacdFast = macdFast, MacdSlow = macdSlow, MacdSignal = macdSignal, TangleSeparationTicks = tangleSeparationTicks, TangleSlopeTicks = tangleSlopeTicks, RequireThreeBarMomentum = requireThreeBarMomentum, RequireDirectionalRelease = requireDirectionalRelease, EntryBufferTicks = entryBufferTicks, AllowKcMidEntry = allowKcMidEntry, AllowMomentumIgnition = allowMomentumIgnition, AllowMacdCrossEntry = allowMacdCrossEntry, AllowBarBreakEntry = allowBarBreakEntry, AllowTrendContinuationEntry = allowTrendContinuationEntry, AllowPartialSqueezeEntry = allowPartialSqueezeEntry, BlockFullSqueezeOnly = blockFullSqueezeOnly, UseIntrabarEntries = useIntrabarEntries, RequireBounceForContinuation = requireBounceForContinuation, RequireKcMidTrend = requireKcMidTrend, MinHistStepTicks = minHistStepTicks, EntryCooldownBars = entryCooldownBars, KcBreakConfirmBars = kcBreakConfirmBars, MinBarsInTrade = minBarsInTrade, RequireMomentumFadeForClose = requireMomentumFadeForClose, UseExhaustionForClose = useExhaustionForClose, UseAtrTrail = useAtrTrail, AtrTrailMultiplier = atrTrailMultiplier, UseIntrabarExits = useIntrabarExits, ShowExitWarning = showExitWarning, EnableExitWarningAlerts = enableExitWarningAlerts, UseProfitGivebackExit = useProfitGivebackExit, MinProfitForGiveback = minProfitForGiveback, ProfitGivebackTicks = profitGivebackTicks, EnableAlerts = enableAlerts, ShowSignals = showSignals, ShowSignalLabels = showSignalLabels, ShowCloseSignals = showCloseSignals, ShowCloseLabels = showCloseLabels, ShowCloseReason = showCloseReason, ShowTradeLines = showTradeLines, ShowReleaseMarker = showReleaseMarker, ShowStatsPanel = showStatsPanel, ShowLiveSituationPanel = showLiveSituationPanel, ShowKcMid = showKcMid, EnableCloseAlerts = enableCloseAlerts, ShowEntryTrigger = showEntryTrigger, ShowSqueezeBackground = showSqueezeBackground, ArrowOffsetTicks = arrowOffsetTicks, LabelOffsetTicks = labelOffsetTicks, CloseOffsetTicks = closeOffsetTicks }, input, ref cacheMES500TSqueezeMomentumV39Light);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSqueezeMomentumV39Light MES500TSqueezeMomentumV39Light(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, int maxMarkerHistoryBars, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV39Light(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, maxMarkerHistoryBars, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public Indicators.MES500TSqueezeMomentumV39Light MES500TSqueezeMomentumV39Light(ISeries<double> input , bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, int maxMarkerHistoryBars, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV39Light(input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, maxMarkerHistoryBars, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}
	}
