...
{
	public class MES500TSqueezeMomentumV36 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500T Squeeze Momentum V3.6 - manual režim: CLOSE až po hold okně, více času na obchod.";
				Name = "MES500TSqueezeMomentumV36";
			...
			bool macdTangle)
		{
			...
			string tag = "MES500TV36_Ring_" + CurrentBar;
		...
		{
			string tag = "MES500TV36_Hint_" + CurrentBar;
		...
		private void ResetTradeTracking()
		{
			...
			postPeakMaxTicks = 0;
			openIsReEntry = false;
			RemoveDrawObject("MES500TV36_Trail");
			RemoveDrawObject("MES500TV36_Char");
			RemoveDrawObject("MES500TV36_Char_Sub");
			RemoveDrawObject("MES500TV36_Char_Hint");
		}
		...
		private void UpdateLiveTradeCharacter(bool isLong, double kcMid, double hist0, double hist1, double hist2)
		{
			...
			Draw.Text(this, "MES500TV36_Char", false, mainLabel, 0, labelY, 0, brush,
				warningFont, TextAlignment.Center, Brushes.Black, Brushes.DimGray, 100);
			DrawSubHint("MES500TV36_Char_Hint", 0, labelY, isLong, hint, brush);
			RequestChartRefresh();
		}
		...
		private void DrawActiveTrailLine(bool isLong)
		{
			...
			Draw.Line(this, "MES500TV36_Trail", false, 1, trailStopPrice, 0, trailStopPrice,
		...
		private void UpdateRecoveryWatch()
		{
			...
			{
				recoveryWatchActive = false;
				RemoveDrawObject("MES500TV36_Missed");
				return;
			}
			...
			if (favorable >= RunnerTargetTicks && !recoveryMissShown)
			{
				...
				Draw.TextFixed(this, "MES500TV36_Missed", text, TextPosition.BottomLeft,
		...
		{
			RemoveDrawObject("MES500TV36_Situation");
		...
		private void DrawExitWarning(int tradeId, bool wasLong, CloseReason reason)
		{
			...
			string tag = "MES500TV36_" + tradeId + "_ExitWarn";
		...
		private void DrawReleaseMarker(SqueezeReleaseDirection direction)
		{
			...
			string tag = "MES500TV36_Release_" + CurrentBar;
		...
		{
			string tag = "MES500TV36_" + tradeId + "_Long";
		...
		{
			string tag = "MES500TV36_" + tradeId + "_Short";
		...
		{
			string tag = "MES500TV36_" + tradeId + "_Close";
		...
		private void DrawStatsPanel()
		{
			...
			Draw.TextFixed(this, "MES500TV36_Stats", statsText, TextPosition.TopRight,
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSqueezeMomentumV36[] cacheMES500TSqueezeMomentumV36;
		public MES500TSqueezeMomentumV36 MES500TSqueezeMomentumV36(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return MES500TSqueezeMomentumV36(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public MES500TSqueezeMomentumV36 MES500TSqueezeMomentumV36(ISeries<double> input, bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			if (cacheMES500TSqueezeMomentumV36 != null)
				for (int idx = 0; idx < cacheMES500TSqueezeMomentumV36.Length; idx++)
					if (cacheMES500TSqueezeMomentumV36[idx] != null && cacheMES500TSqueezeMomentumV36[idx].ManualTradeMode == manualTradeMode && cacheMES500TSqueezeMomentumV36[idx].MinBarsBeforeSoftClose == minBarsBeforeSoftClose && cacheMES500TSqueezeMomentumV36[idx].AllowEarlyFlipClose == allowEarlyFlipClose && cacheMES500TSqueezeMomentumV36[idx].ShowHoldWindowTag == showHoldWindowTag && cacheMES500TSqueezeMomentumV36[idx].SuppressExitWarningInHold == suppressExitWarningInHold && cacheMES500TSqueezeMomentumV36[idx].ShowApproachRing == showApproachRing && cacheMES500TSqueezeMomentumV36[idx].ApproachRingMinScore == approachRingMinScore && cacheMES500TSqueezeMomentumV36[idx].ApproachRingSizeTicks == approachRingSizeTicks && cacheMES500TSqueezeMomentumV36[idx].ApproachRingMinOpacity == approachRingMinOpacity && cacheMES500TSqueezeMomentumV36[idx].ApproachRingMaxOpacity == approachRingMaxOpacity && cacheMES500TSqueezeMomentumV36[idx].EnableScreenPopup == enableScreenPopup && cacheMES500TSqueezeMomentumV36[idx].PopupDurationSec == popupDurationSec && cacheMES500TSqueezeMomentumV36[idx].PopupOnApproach == popupOnApproach && cacheMES500TSqueezeMomentumV36[idx].PopupOnEntry == popupOnEntry && cacheMES500TSqueezeMomentumV36[idx].PopupOnClose == popupOnClose && cacheMES500TSqueezeMomentumV36[idx].ShowCloseHoldAdvice == showCloseHoldAdvice && cacheMES500TSqueezeMomentumV36[idx].EnableSignalAutoCorrect == enableSignalAutoCorrect && cacheMES500TSqueezeMomentumV36[idx].SignalReviewBars == signalReviewBars && cacheMES500TSqueezeMomentumV36[idx].SignalMinBarsBeforeReview == signalMinBarsBeforeReview && cacheMES500TSqueezeMomentumV36[idx].FalseSignalAdverseTicks == falseSignalAdverseTicks && cacheMES500TSqueezeMomentumV36[idx].SignalConfirmTicks == signalConfirmTicks && cacheMES500TSqueezeMomentumV36[idx].BlockWeakReEntrySignals == blockWeakReEntrySignals && cacheMES500TSqueezeMomentumV36[idx].ReEntryMinBarsSinceClose == reEntryMinBarsSinceClose && cacheMES500TSqueezeMomentumV36[idx].ReEntryRequireGradeBOrBetter == reEntryRequireGradeBOrBetter && cacheMES500TSqueezeMomentumV36[idx].BlockReEntryAfterChopClose == blockReEntryAfterChopClose && cacheMES500TSqueezeMomentumV36[idx].BlockReEntryAfterLossClose == blockReEntryAfterLossClose && cacheMES500TSqueezeMomentumV36[idx].ReEntryApproachMinScore == reEntryApproachMinScore && cacheMES500TSqueezeMomentumV36[idx].CntMinKcMidSlopeBars == cntMinKcMidSlopeBars && cacheMES500TSqueezeMomentumV36[idx].ShowTrailLine == showTrailLine && cacheMES500TSqueezeMomentumV36[idx].ShowSignalGrade == showSignalGrade && cacheMES500TSqueezeMomentumV36[idx].ShowTradeCharacter == showTradeCharacter && cacheMES500TSqueezeMomentumV36[idx].ChopQuickPeakTicks == chopQuickPeakTicks && cacheMES500TSqueezeMomentumV36[idx].ChopGivebackTicks == chopGivebackTicks && cacheMES500TSqueezeMomentumV36[idx].RunnerTargetTicks == runnerTargetTicks && cacheMES500TSqueezeMomentumV36[idx].RecoveryWindowSec == recoveryWindowSec && cacheMES500TSqueezeMomentumV36[idx].EnableTaskbarFlash == enableTaskbarFlash && cacheMES500TSqueezeMomentumV36[idx].FlashOnEntry == flashOnEntry && cacheMES500TSqueezeMomentumV36[idx].FlashOnClose == flashOnClose && cacheMES500TSqueezeMomentumV36[idx].FlashOnApproach == flashOnApproach && cacheMES500TSqueezeMomentumV36[idx].ShowApproachHints == showApproachHints && cacheMES500TSqueezeMomentumV36[idx].ShowApproachLabels == showApproachLabels && cacheMES500TSqueezeMomentumV36[idx].ApproachMinScore == approachMinScore && cacheMES500TSqueezeMomentumV36[idx].ApproachNearTicks == approachNearTicks && cacheMES500TSqueezeMomentumV36[idx].ApproachOffsetTicks == approachOffsetTicks && cacheMES500TSqueezeMomentumV36[idx].ReEntryLookbackBars == reEntryLookbackBars && cacheMES500TSqueezeMomentumV36[idx].BbPeriod == bbPeriod && cacheMES500TSqueezeMomentumV36[idx].BbStdDev == bbStdDev && cacheMES500TSqueezeMomentumV36[idx].KcPeriod == kcPeriod && cacheMES500TSqueezeMomentumV36[idx].KcMultiplier == kcMultiplier && cacheMES500TSqueezeMomentumV36[idx].MacdFast == macdFast && cacheMES500TSqueezeMomentumV36[idx].MacdSlow == macdSlow && cacheMES500TSqueezeMomentumV36[idx].MacdSignal == macdSignal && cacheMES500TSqueezeMomentumV36[idx].TangleSeparationTicks == tangleSeparationTicks && cacheMES500TSqueezeMomentumV36[idx].TangleSlopeTicks == tangleSlopeTicks && cacheMES500TSqueezeMomentumV36[idx].RequireThreeBarMomentum == requireThreeBarMomentum && cacheMES500TSqueezeMomentumV36[idx].RequireDirectionalRelease == requireDirectionalRelease && cacheMES500TSqueezeMomentumV36[idx].EntryBufferTicks == entryBufferTicks && cacheMES500TSqueezeMomentumV36[idx].AllowKcMidEntry == allowKcMidEntry && cacheMES500TSqueezeMomentumV36[idx].AllowMomentumIgnition == allowMomentumIgnition && cacheMES500TSqueezeMomentumV36[idx].AllowMacdCrossEntry == allowMacdCrossEntry && cacheMES500TSqueezeMomentumV36[idx].AllowBarBreakEntry == allowBarBreakEntry && cacheMES500TSqueezeMomentumV36[idx].AllowTrendContinuationEntry == allowTrendContinuationEntry && cacheMES500TSqueezeMomentumV36[idx].AllowPartialSqueezeEntry == allowPartialSqueezeEntry && cacheMES500TSqueezeMomentumV36[idx].BlockFullSqueezeOnly == blockFullSqueezeOnly && cacheMES500TSqueezeMomentumV36[idx].UseIntrabarEntries == useIntrabarEntries && cacheMES500TSqueezeMomentumV36[idx].RequireBounceForContinuation == requireBounceForContinuation && cacheMES500TSqueezeMomentumV36[idx].RequireKcMidTrend == requireKcMidTrend && cacheMES500TSqueezeMomentumV36[idx].MinHistStepTicks == minHistStepTicks && cacheMES500TSqueezeMomentumV36[idx].EntryCooldownBars == entryCooldownBars && cacheMES500TSqueezeMomentumV36[idx].KcBreakConfirmBars == kcBreakConfirmBars && cacheMES500TSqueezeMomentumV36[idx].MinBarsInTrade == minBarsInTrade && cacheMES500TSqueezeMomentumV36[idx].RequireMomentumFadeForClose == requireMomentumFadeForClose && cacheMES500TSqueezeMomentumV36[idx].UseExhaustionForClose == useExhaustionForClose && cacheMES500TSqueezeMomentumV36[idx].UseAtrTrail == useAtrTrail && cacheMES500TSqueezeMomentumV36[idx].AtrTrailMultiplier == atrTrailMultiplier && cacheMES500TSqueezeMomentumV36[idx].UseIntrabarExits == useIntrabarExits && cacheMES500TSqueezeMomentumV36[idx].ShowExitWarning == showExitWarning && cacheMES500TSqueezeMomentumV36[idx].EnableExitWarningAlerts == enableExitWarningAlerts && cacheMES500TSqueezeMomentumV36[idx].UseProfitGivebackExit == useProfitGivebackExit && cacheMES500TSqueezeMomentumV36[idx].MinProfitForGiveback == minProfitForGiveback && cacheMES500TSqueezeMomentumV36[idx].ProfitGivebackTicks == profitGivebackTicks && cacheMES500TSqueezeMomentumV36[idx].EnableAlerts == enableAlerts && cacheMES500TSqueezeMomentumV36[idx].ShowSignals == showSignals && cacheMES500TSqueezeMomentumV36[idx].ShowSignalLabels == showSignalLabels && cacheMES500TSqueezeMomentumV36[idx].ShowCloseSignals == showCloseSignals && cacheMES500TSqueezeMomentumV36[idx].ShowCloseLabels == showCloseLabels && cacheMES500TSqueezeMomentumV36[idx].ShowCloseReason == showCloseReason && cacheMES500TSqueezeMomentumV36[idx].ShowTradeLines == showTradeLines && cacheMES500TSqueezeMomentumV36[idx].ShowReleaseMarker == showReleaseMarker && cacheMES500TSqueezeMomentumV36[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSqueezeMomentumV36[idx].ShowLiveSituationPanel == showLiveSituationPanel && cacheMES500TSqueezeMomentumV36[idx].ShowKcMid == showKcMid && cacheMES500TSqueezeMomentumV36[idx].EnableCloseAlerts == enableCloseAlerts && cacheMES500TSqueezeMomentumV36[idx].ShowEntryTrigger == showEntryTrigger && cacheMES500TSqueezeMomentumV36[idx].ShowSqueezeBackground == showSqueezeBackground && cacheMES500TSqueezeMomentumV36[idx].ArrowOffsetTicks == arrowOffsetTicks && cacheMES500TSqueezeMomentumV36[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSqueezeMomentumV36[idx].CloseOffsetTicks == closeOffsetTicks && cacheMES500TSqueezeMomentumV36[idx].EqualsInput(input))
						return cacheMES500TSqueezeMomentumV36[idx];
			return CacheIndicator<MES500TSqueezeMomentumV36>(new MES500TSqueezeMomentumV36(){ ManualTradeMode = manualTradeMode, MinBarsBeforeSoftClose = minBarsBeforeSoftClose, AllowEarlyFlipClose = allowEarlyFlipClose, ShowHoldWindowTag = showHoldWindowTag, SuppressExitWarningInHold = suppressExitWarningInHold, ShowApproachRing = showApproachRing, ApproachRingMinScore = approachRingMinScore, ApproachRingSizeTicks = approachRingSizeTicks, ApproachRingMinOpacity = approachRingMinOpacity, ApproachRingMaxOpacity = approachRingMaxOpacity, EnableScreenPopup = enableScreenPopup, PopupDurationSec = popupDurationSec, PopupOnApproach = popupOnApproach, PopupOnEntry = popupOnEntry, PopupOnClose = popupOnClose, ShowCloseHoldAdvice = showCloseHoldAdvice, EnableSignalAutoCorrect = enableSignalAutoCorrect, SignalReviewBars = signalReviewBars, SignalMinBarsBeforeReview = signalMinBarsBeforeReview, FalseSignalAdverseTicks = falseSignalAdverseTicks, SignalConfirmTicks = signalConfirmTicks, BlockWeakReEntrySignals = blockWeakReEntrySignals, ReEntryMinBarsSinceClose = reEntryMinBarsSinceClose, ReEntryRequireGradeBOrBetter = reEntryRequireGradeBOrBetter, BlockReEntryAfterChopClose = blockReEntryAfterChopClose, BlockReEntryAfterLossClose = blockReEntryAfterLossClose, ReEntryApproachMinScore = reEntryApproachMinScore, CntMinKcMidSlopeBars = cntMinKcMidSlopeBars, ShowTrailLine = showTrailLine, ShowSignalGrade = showSignalGrade, ShowTradeCharacter = showTradeCharacter, ChopQuickPeakTicks = chopQuickPeakTicks, ChopGivebackTicks = chopGivebackTicks, RunnerTargetTicks = runnerTargetTicks, RecoveryWindowSec = recoveryWindowSec, EnableTaskbarFlash = enableTaskbarFlash, FlashOnEntry = flashOnEntry, FlashOnClose = flashOnClose, FlashOnApproach = flashOnApproach, ShowApproachHints = showApproachHints, ShowApproachLabels = showApproachLabels, ApproachMinScore = approachMinScore, ApproachNearTicks = approachNearTicks, ApproachOffsetTicks = approachOffsetTicks, ReEntryLookbackBars = reEntryLookbackBars, BbPeriod = bbPeriod, BbStdDev = bbStdDev, KcPeriod = kcPeriod, KcMultiplier = kcMultiplier, MacdFast = macdFast, MacdSlow = macdSlow, MacdSignal = macdSignal, TangleSeparationTicks = tangleSeparationTicks, TangleSlopeTicks = tangleSlopeTicks, RequireThreeBarMomentum = requireThreeBarMomentum, RequireDirectionalRelease = requireDirectionalRelease, EntryBufferTicks = entryBufferTicks, AllowKcMidEntry = allowKcMidEntry, AllowMomentumIgnition = allowMomentumIgnition, AllowMacdCrossEntry = allowMacdCrossEntry, AllowBarBreakEntry = allowBarBreakEntry, AllowTrendContinuationEntry = allowTrendContinuationEntry, AllowPartialSqueezeEntry = allowPartialSqueezeEntry, BlockFullSqueezeOnly = blockFullSqueezeOnly, UseIntrabarEntries = useIntrabarEntries, RequireBounceForContinuation = requireBounceForContinuation, RequireKcMidTrend = requireKcMidTrend, MinHistStepTicks = minHistStepTicks, EntryCooldownBars = entryCooldownBars, KcBreakConfirmBars = kcBreakConfirmBars, MinBarsInTrade = minBarsInTrade, RequireMomentumFadeForClose = requireMomentumFadeForClose, UseExhaustionForClose = useExhaustionForClose, UseAtrTrail = useAtrTrail, AtrTrailMultiplier = atrTrailMultiplier, UseIntrabarExits = useIntrabarExits, ShowExitWarning = showExitWarning, EnableExitWarningAlerts = enableExitWarningAlerts, UseProfitGivebackExit = useProfitGivebackExit, MinProfitForGiveback = minProfitForGiveback, ProfitGivebackTicks = profitGivebackTicks, EnableAlerts = enableAlerts, ShowSignals = showSignals, ShowSignalLabels = showSignalLabels, ShowCloseSignals = showCloseSignals, ShowCloseLabels = showCloseLabels, ShowCloseReason = showCloseReason, ShowTradeLines = showTradeLines, ShowReleaseMarker = showReleaseMarker, ShowStatsPanel = showStatsPanel, ShowLiveSituationPanel = showLiveSituationPanel, ShowKcMid = showKcMid, EnableCloseAlerts = enableCloseAlerts, ShowEntryTrigger = showEntryTrigger, ShowSqueezeBackground = showSqueezeBackground, ArrowOffsetTicks = arrowOffsetTicks, LabelOffsetTicks = labelOffsetTicks, CloseOffsetTicks = closeOffsetTicks }, input, ref cacheMES500TSqueezeMomentumV36);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSqueezeMomentumV36 MES500TSqueezeMomentumV36(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV36(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public Indicators.MES500TSqueezeMomentumV36 MES500TSqueezeMomentumV36(ISeries<double> input , bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV36(input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}
	}
