...
{
	/// <summary>
	/// MES500TSwingMapV5 — V4 + VSTUP až po skončení korekce (ne na začátku).
	/// </summary>
	public class MES500TSwingMapV5 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500TSwingMapV5 — VSTUP až po korekci + statistiky VSTUP→◆ PEAK.";
				Name        = "MES500TSwingMapV5";
		...
		private void UpdateStatsPanel()
		{
			if (!ShowStatsPanel)
			{
				Draw.TextFixed(this, "MES500TSMV5_Stats", string.Empty,
			...
			Draw.TextFixed(this, "MES500TSMV5_Stats", text,
		...
		private void UpdateActionPanel()
		{
			...
			Draw.TextFixed(this, "MES500TSMV5_Action", text,
		...
		private void DrawEntryMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5_Entry_" + CurrentBar;
		...
		private void DrawPeakSoftMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5_PeakSoft_" + CurrentBar;
		...
		private void DrawPeakConfirmedMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5_Peak_" + CurrentBar;
		...
		private void DrawSwingLine(int barA, double priceA, int barB, double priceB)
		{
			...
			string tag = "MES500TSMV5_Line_" + barA + "_" + barB;
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSwingMapV5[] cacheMES500TSwingMapV5;
		public MES500TSwingMapV5 MES500TSwingMapV5(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return MES500TSwingMapV5(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public MES500TSwingMapV5 MES500TSwingMapV5(ISeries<double> input, int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			if (cacheMES500TSwingMapV5 != null)
				for (int idx = 0; idx < cacheMES500TSwingMapV5.Length; idx++)
					if (cacheMES500TSwingMapV5[idx] != null && cacheMES500TSwingMapV5[idx].MinSwingTicks == minSwingTicks && cacheMES500TSwingMapV5[idx].CorrectionRatio == correctionRatio && cacheMES500TSwingMapV5[idx].ConfirmBars == confirmBars && cacheMES500TSwingMapV5[idx].AtrPeriod == atrPeriod && cacheMES500TSwingMapV5[idx].AtrMultiplier == atrMultiplier && cacheMES500TSwingMapV5[idx].ShowSwingLines == showSwingLines && cacheMES500TSwingMapV5[idx].ShowStrengthPanel == showStrengthPanel && cacheMES500TSwingMapV5[idx].ShowActionPanel == showActionPanel && cacheMES500TSwingMapV5[idx].ShowEntryMarkers == showEntryMarkers && cacheMES500TSwingMapV5[idx].ShowEntryConnector == showEntryConnector && cacheMES500TSwingMapV5[idx].ShowPeakMarkers == showPeakMarkers && cacheMES500TSwingMapV5[idx].ShowPeakSoftMarkers == showPeakSoftMarkers && cacheMES500TSwingMapV5[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSwingMapV5[idx].PeakMinTicks == peakMinTicks && cacheMES500TSwingMapV5[idx].PeakPullbackTicks == peakPullbackTicks && cacheMES500TSwingMapV5[idx].PeakRequireBreak == peakRequireBreak && cacheMES500TSwingMapV5[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSwingMapV5[idx].ContractCount == contractCount && cacheMES500TSwingMapV5[idx].StatsLookbackHours == statsLookbackHours && cacheMES500TSwingMapV5[idx].MaxStatsTrades == maxStatsTrades && cacheMES500TSwingMapV5[idx].EqualsInput(input))
						return cacheMES500TSwingMapV5[idx];
			return CacheIndicator<MES500TSwingMapV5>(new MES500TSwingMapV5(){ MinSwingTicks = minSwingTicks, CorrectionRatio = correctionRatio, ConfirmBars = confirmBars, AtrPeriod = atrPeriod, AtrMultiplier = atrMultiplier, ShowSwingLines = showSwingLines, ShowStrengthPanel = showStrengthPanel, ShowActionPanel = showActionPanel, ShowEntryMarkers = showEntryMarkers, ShowEntryConnector = showEntryConnector, ShowPeakMarkers = showPeakMarkers, ShowPeakSoftMarkers = showPeakSoftMarkers, LabelOffsetTicks = labelOffsetTicks, PeakMinTicks = peakMinTicks, PeakPullbackTicks = peakPullbackTicks, PeakRequireBreak = peakRequireBreak, ShowStatsPanel = showStatsPanel, ContractCount = contractCount, StatsLookbackHours = statsLookbackHours, MaxStatsTrades = maxStatsTrades }, input, ref cacheMES500TSwingMapV5);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSwingMapV5 MES500TSwingMapV5(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV5(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public Indicators.MES500TSwingMapV5 MES500TSwingMapV5(ISeries<double> input , int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV5(input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}
	}
