...
{
	/// <summary>
	/// MES500TSwingMapV5Light — V5 pro slabší PC (statistiky za 1h).
	/// </summary>
	public class MES500TSwingMapV5Light : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500TSwingMapV5Light — V5 Light, statistiky za 1h.";
				Name        = "MES500TSwingMapV5Light";
		...
		private SwingType ProcessSeekingHigh(double minMove)
		{
			...
			if (completedType == SwingType.BuyImpulse)
			{
				...
				SignalEntry(SwingDir.Long,
					correctionActive ? "Po korekci ↑ BUY — VSTUP" : "↑ BUY potvrzeno — VSTUP",
					lastPivotPrice, upLegTicks);
				correctionActive = false;
			}
			...
			runningExtremeBar = CurrentBar;
			segmentStartBar = CurrentBar;
			if (completedType == SwingType.Correction)
			{
				phase = TrendPhase.CorrectionAfterSell;
				segmentType = SwingType.Correction;
			}
			else
			{
		...
		private SwingType ProcessSeekingLow(double minMove)
		{
			...
			if (completedType == SwingType.SellImpulse)
			{
				...
				SignalEntry(SwingDir.Short,
					correctionActive ? "Po korekci ↓ SELL — VSTUP" : "↓ SELL potvrzeno — VSTUP",
					lastPivotPrice, downLegTicks);
				correctionActive = false;
			}
			else if (completedType == SwingType.Correction)
			{
		...
		{
			// Vzestup po SELL = korekce — VSTUP až po potvrzeném pivotu, ne uprostřed odrazu
			if (lastImpulseDir == SwingDir.Short && lastPivotKind == PivotKind.Low)
			{
				SetAction("↩ KOREKCE — NEVSTUPOVAT", "Čekej VSTUP až po skončení korekce", brushCorrection);
				return SwingType.Correction;
			}

			if (correctionActive || phase == TrendPhase.CorrectionAfterSell)
			{
				SetAction("↩ KOREKCE — NEVSTUPOVAT", "Čekej VSTUP až po skončení korekce", brushCorrection);
				return SwingType.Correction;
			}
		...
		{
			// Pokles po BUY = korekce — VSTUP až po potvrzeném pivotu
			if (lastImpulseDir == SwingDir.Long && lastPivotKind == PivotKind.High)
			{
				SetAction("↩ KOREKCE — NEVSTUPOVAT", "Čekej VSTUP až po skončení korekce", brushCorrection);
				return SwingType.Correction;
			}

			if (correctionActive || phase == TrendPhase.CorrectionAfterBuy)
			{
				SetAction("↩ KOREKCE — NEVSTUPOVAT", "Čekej VSTUP až po skončení korekce", brushCorrection);
				return SwingType.Correction;
			}
		...
		private void RecordConfirmedPeak(SwingDir dir, double peakLevel, double exitClose)
		{
			...
			// Win = skutečný zisk při exitu (close ◆ PEAK vs close VSTUP) — ne runningExtreme,
			// které je max celého legu a téměř vždy „vyhrává“ i když exit je ve ztrátě.
		...
		private void UpdateStatsPanel()
		{
			if (!ShowStatsPanel)
			{
				Draw.TextFixed(this, "MES500TSMV5Light_Stats", string.Empty,
			...
			Draw.TextFixed(this, "MES500TSMV5Light_Stats", text,
		...
		private void UpdateActionPanel()
		{
			...
			Draw.TextFixed(this, "MES500TSMV5Light_Action", text,
		...
		private void DrawEntryMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5Light_Entry_" + CurrentBar;
		...
		private void DrawPeakSoftMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5Light_PeakSoft_" + CurrentBar;
		...
		private void DrawPeakConfirmedMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV5Light_Peak_" + CurrentBar;
		...
		private void DrawSwingLine(int barA, double priceA, int barB, double priceB)
		{
			...
			string tag = "MES500TSMV5Light_Line_" + barA + "_" + barB;
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSwingMapV5Light[] cacheMES500TSwingMapV5Light;
		public MES500TSwingMapV5Light MES500TSwingMapV5Light(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return MES500TSwingMapV5Light(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public MES500TSwingMapV5Light MES500TSwingMapV5Light(ISeries<double> input, int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			if (cacheMES500TSwingMapV5Light != null)
				for (int idx = 0; idx < cacheMES500TSwingMapV5Light.Length; idx++)
					if (cacheMES500TSwingMapV5Light[idx] != null && cacheMES500TSwingMapV5Light[idx].MinSwingTicks == minSwingTicks && cacheMES500TSwingMapV5Light[idx].CorrectionRatio == correctionRatio && cacheMES500TSwingMapV5Light[idx].ConfirmBars == confirmBars && cacheMES500TSwingMapV5Light[idx].AtrPeriod == atrPeriod && cacheMES500TSwingMapV5Light[idx].AtrMultiplier == atrMultiplier && cacheMES500TSwingMapV5Light[idx].ShowSwingLines == showSwingLines && cacheMES500TSwingMapV5Light[idx].ShowStrengthPanel == showStrengthPanel && cacheMES500TSwingMapV5Light[idx].ShowActionPanel == showActionPanel && cacheMES500TSwingMapV5Light[idx].ShowEntryMarkers == showEntryMarkers && cacheMES500TSwingMapV5Light[idx].ShowEntryConnector == showEntryConnector && cacheMES500TSwingMapV5Light[idx].ShowPeakMarkers == showPeakMarkers && cacheMES500TSwingMapV5Light[idx].ShowPeakSoftMarkers == showPeakSoftMarkers && cacheMES500TSwingMapV5Light[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSwingMapV5Light[idx].PeakMinTicks == peakMinTicks && cacheMES500TSwingMapV5Light[idx].PeakPullbackTicks == peakPullbackTicks && cacheMES500TSwingMapV5Light[idx].PeakRequireBreak == peakRequireBreak && cacheMES500TSwingMapV5Light[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSwingMapV5Light[idx].ContractCount == contractCount && cacheMES500TSwingMapV5Light[idx].StatsLookbackHours == statsLookbackHours && cacheMES500TSwingMapV5Light[idx].MaxStatsTrades == maxStatsTrades && cacheMES500TSwingMapV5Light[idx].EqualsInput(input))
						return cacheMES500TSwingMapV5Light[idx];
			return CacheIndicator<MES500TSwingMapV5Light>(new MES500TSwingMapV5Light(){ MinSwingTicks = minSwingTicks, CorrectionRatio = correctionRatio, ConfirmBars = confirmBars, AtrPeriod = atrPeriod, AtrMultiplier = atrMultiplier, ShowSwingLines = showSwingLines, ShowStrengthPanel = showStrengthPanel, ShowActionPanel = showActionPanel, ShowEntryMarkers = showEntryMarkers, ShowEntryConnector = showEntryConnector, ShowPeakMarkers = showPeakMarkers, ShowPeakSoftMarkers = showPeakSoftMarkers, LabelOffsetTicks = labelOffsetTicks, PeakMinTicks = peakMinTicks, PeakPullbackTicks = peakPullbackTicks, PeakRequireBreak = peakRequireBreak, ShowStatsPanel = showStatsPanel, ContractCount = contractCount, StatsLookbackHours = statsLookbackHours, MaxStatsTrades = maxStatsTrades }, input, ref cacheMES500TSwingMapV5Light);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSwingMapV5Light MES500TSwingMapV5Light(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV5Light(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public Indicators.MES500TSwingMapV5Light MES500TSwingMapV5Light(ISeries<double> input , int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV5Light(input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}
	}
