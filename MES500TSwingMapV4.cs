...
{
	/// <summary>
	/// MES500TSwingMapV4 — V3 + statistiky VSTUP → ◆ PEAK (win rate, ticky, USD).
	/// </summary>
	public class MES500TSwingMapV4 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500TSwingMapV4 — V3 + statistiky VSTUP→◆ PEAK (win rate, USD).";
				Name        = "MES500TSwingMapV4";
		...
		private void UpdateStatsPanel()
		{
			if (!ShowStatsPanel)
			{
				Draw.TextFixed(this, "MES500TSMV4_Stats", string.Empty,
			...
			Draw.TextFixed(this, "MES500TSMV4_Stats", text,
		...
		private void UpdateActionPanel()
		{
			...
			Draw.TextFixed(this, "MES500TSMV4_Action", text,
		...
		private void DrawEntryMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV4_Entry_" + CurrentBar;
		...
		private void DrawPeakSoftMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV4_PeakSoft_" + CurrentBar;
		...
		private void DrawPeakConfirmedMarker(SwingDir dir)
		{
			...
			string barTag = "MES500TSMV4_Peak_" + CurrentBar;
		...
		private void DrawSwingLine(int barA, double priceA, int barB, double priceB)
		{
			...
			string tag = "MES500TSMV4_Line_" + barA + "_" + barB;
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSwingMapV4[] cacheMES500TSwingMapV4;
		public MES500TSwingMapV4 MES500TSwingMapV4(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return MES500TSwingMapV4(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public MES500TSwingMapV4 MES500TSwingMapV4(ISeries<double> input, int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			if (cacheMES500TSwingMapV4 != null)
				for (int idx = 0; idx < cacheMES500TSwingMapV4.Length; idx++)
					if (cacheMES500TSwingMapV4[idx] != null && cacheMES500TSwingMapV4[idx].MinSwingTicks == minSwingTicks && cacheMES500TSwingMapV4[idx].CorrectionRatio == correctionRatio && cacheMES500TSwingMapV4[idx].ConfirmBars == confirmBars && cacheMES500TSwingMapV4[idx].AtrPeriod == atrPeriod && cacheMES500TSwingMapV4[idx].AtrMultiplier == atrMultiplier && cacheMES500TSwingMapV4[idx].ShowSwingLines == showSwingLines && cacheMES500TSwingMapV4[idx].ShowStrengthPanel == showStrengthPanel && cacheMES500TSwingMapV4[idx].ShowActionPanel == showActionPanel && cacheMES500TSwingMapV4[idx].ShowEntryMarkers == showEntryMarkers && cacheMES500TSwingMapV4[idx].ShowEntryConnector == showEntryConnector && cacheMES500TSwingMapV4[idx].ShowPeakMarkers == showPeakMarkers && cacheMES500TSwingMapV4[idx].ShowPeakSoftMarkers == showPeakSoftMarkers && cacheMES500TSwingMapV4[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSwingMapV4[idx].PeakMinTicks == peakMinTicks && cacheMES500TSwingMapV4[idx].PeakPullbackTicks == peakPullbackTicks && cacheMES500TSwingMapV4[idx].PeakRequireBreak == peakRequireBreak && cacheMES500TSwingMapV4[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSwingMapV4[idx].ContractCount == contractCount && cacheMES500TSwingMapV4[idx].StatsLookbackHours == statsLookbackHours && cacheMES500TSwingMapV4[idx].MaxStatsTrades == maxStatsTrades && cacheMES500TSwingMapV4[idx].EqualsInput(input))
						return cacheMES500TSwingMapV4[idx];
			return CacheIndicator<MES500TSwingMapV4>(new MES500TSwingMapV4(){ MinSwingTicks = minSwingTicks, CorrectionRatio = correctionRatio, ConfirmBars = confirmBars, AtrPeriod = atrPeriod, AtrMultiplier = atrMultiplier, ShowSwingLines = showSwingLines, ShowStrengthPanel = showStrengthPanel, ShowActionPanel = showActionPanel, ShowEntryMarkers = showEntryMarkers, ShowEntryConnector = showEntryConnector, ShowPeakMarkers = showPeakMarkers, ShowPeakSoftMarkers = showPeakSoftMarkers, LabelOffsetTicks = labelOffsetTicks, PeakMinTicks = peakMinTicks, PeakPullbackTicks = peakPullbackTicks, PeakRequireBreak = peakRequireBreak, ShowStatsPanel = showStatsPanel, ContractCount = contractCount, StatsLookbackHours = statsLookbackHours, MaxStatsTrades = maxStatsTrades }, input, ref cacheMES500TSwingMapV4);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSwingMapV4 MES500TSwingMapV4(int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV4(Input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}

		public Indicators.MES500TSwingMapV4 MES500TSwingMapV4(ISeries<double> input , int minSwingTicks, double correctionRatio, int confirmBars, int atrPeriod, double atrMultiplier, bool showSwingLines, bool showStrengthPanel, bool showActionPanel, bool showEntryMarkers, bool showEntryConnector, bool showPeakMarkers, bool showPeakSoftMarkers, int labelOffsetTicks, int peakMinTicks, int peakPullbackTicks, bool peakRequireBreak, bool showStatsPanel, int contractCount, int statsLookbackHours, int maxStatsTrades)
		{
			return indicator.MES500TSwingMapV4(input, minSwingTicks, correctionRatio, confirmBars, atrPeriod, atrMultiplier, showSwingLines, showStrengthPanel, showActionPanel, showEntryMarkers, showEntryConnector, showPeakMarkers, showPeakSoftMarkers, labelOffsetTicks, peakMinTicks, peakPullbackTicks, peakRequireBreak, showStatsPanel, contractCount, statsLookbackHours, maxStatsTrades);
		}
	}
