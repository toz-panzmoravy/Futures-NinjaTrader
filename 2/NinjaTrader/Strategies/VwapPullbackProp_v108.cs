...
{
	/// <summary>
	/// VWAP Pullback Prop AOS v1.08 – HIGH ACTIVITY preset (TICK 125).
	/// Uvolněné signálové filtry + širší okno pro cíl 7–10 obchodů/týden.
	/// End 21:30, blok 17h, SL 55t / PT 65t, LONG-only.
	/// </summary>
	public class VwapPullbackProp_v108 : Strategy
	{
		#region Enums
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "VWAP Pullback Prop v1.08 – aktivní preset (TICK 125), end 21:30, SL 55 / PT 65 tick.";
				Name = "VwapPullbackProp_v108";
				...
				DailyLossLimit = 500;
				MaxTradesPerDay = 12;
				MaxConsecutiveLossesPerDay = 5;
				CooldownBarsAfterLoss = 1;
				SkipFirstMinutes = 0;
				BlockedEntryHours = "17";

				EntryStartTime = DateTime.Parse("15:30", CultureInfo.InvariantCulture);
				EntryEndTime = DateTime.Parse("21:30", CultureInfo.InvariantCulture);
		...
		private void ValidateTickChart()
		{
			...
			if (BarsPeriod.BarsPeriodType != BarsPeriodType.Tick)
				Print(string.Format("{0}: Optimalizováno pro TICK 125 graf, aktuálně: {1}.", Name, BarsPeriod.BarsPeriodType));
