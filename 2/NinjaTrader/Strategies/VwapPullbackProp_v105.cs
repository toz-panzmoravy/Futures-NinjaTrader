...
{
	/// <summary>
	/// VWAP Pullback Prop AOS v1.05 – ochrana proti chop/krizovým obdobím (Feb–Bře).
	/// v1.04 + ADX filtr, VWAP chop detekce, stop po sérii ztrát, denní loss stop 150 USD.
	/// </summary>
	public class VwapPullbackProp_v105 : Strategy
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "VWAP Pullback Prop v1.05 – chop ochrana (ADX, VWAP range, loss streak stop).";
				...
				// v1.05 – neutralizace chop/krizových období
				UseChopFilter = true;
				MinAdx = 18;
				AdxPeriod = 14;
				UseVwapChopFilter = true;
				VwapChopLookback = 25;
				VwapChopMaxDistanceTicks = 6;
				VwapChopMinNearBars = 18;
				MaxConsecutiveLossesPerDay = 3;
				DailyLossStopAmount = 150;
				RequireVwapSlope = true;
				VwapSlopeLookbackBars = 8;
		...
		private void UpdateSessionVwap()
		{
			...
			sessionVwapBarsAgo = sessionVwap;
			sessionVwap = cumulativeTypicalPriceVolume / cumulativeVolume;
		}
		#endregion

		#region Chop / regime filtry v1.05
		private bool PassesRegimeFilters()
		{
			if (UseChopFilter && adx[0] < MinAdx)
				return false;

			if (UseVwapChopFilter && IsVwapChopZone())
				return false;

			if (RequireVwapSlope && !HasValidVwapSlopeForLong())
				return false;

			return true;
		}

		private bool IsVwapChopZone()
		{
			int lookback = Math.Min(VwapChopLookback, CurrentBar);
			if (lookback <= 0)
				return false;
			...
			{
				if (Math.Abs(Close[i] - sessionVwap) / TickSize <= VwapChopMaxDistanceTicks)
					nearVwapBars++;
			}
			return nearVwapBars >= VwapChopMinNearBars;
		}

		private bool HasValidVwapSlopeForLong()
		{
			if (!RequireVwapSlope || CurrentBar < VwapSlopeLookbackBars)
				return true;
			return Close[0] > Close[VwapSlopeLookbackBars] && sessionVwap >= sessionVwapBarsAgo;
		}
		#endregion
		...
		private void UpdateDailyLossLimit()
		{
			...
			if (!dailyLossLimitHit && dailyPnL <= -Math.Abs(DailyLossLimit))
				dailyLossLimitHit = true;

			if (!dailyChopStopHit && DailyLossStopAmount > 0 && dailyPnL <= -Math.Abs(DailyLossStopAmount))
				dailyChopStopHit = true;
		}

		private bool CanEnterNewTrade()
		{
			if (dailyLossLimitHit || dailyChopStopHit || flatBeforeCloseDone)
				return false;
		...
		protected override void OnExecutionUpdate(Execution execution, string executionId, double price, int quantity, MarketPosition marketPosition, string orderId, DateTime time)
		{
			...
			else if (execution.Order.Name == "Profit target")
				dailyConsecutiveLosses = 0;
		}
		...
		[NinjaScriptProperty][Display(Name = "Use Chop Filter (ADX)", Order = 1, GroupName = "7. Chop ochrana")]
		public bool UseChopFilter { get; set; }

		[NinjaScriptProperty][Range(1, 100)]
		[Display(Name = "Min ADX", Order = 2, GroupName = "7. Chop ochrana")]
		public int MinAdx { get; set; }

		[NinjaScriptProperty][Range(1, 100)]
		[Display(Name = "ADX Period", Order = 3, GroupName = "7. Chop ochrana")]
		public int AdxPeriod { get; set; }

		[NinjaScriptProperty][Display(Name = "Use VWAP Chop Filter", Description = "Blokuje obchod když cena osciluje těsně kolem VWAP.", Order = 4, GroupName = "7. Chop ochrana")]
		public bool UseVwapChopFilter { get; set; }

		[NinjaScriptProperty][Range(5, 100)]
		[Display(Name = "VWAP Chop Lookback", Order = 5, GroupName = "7. Chop ochrana")]
		public int VwapChopLookback { get; set; }

		[NinjaScriptProperty][Range(1, 30)]
		[Display(Name = "VWAP Chop Max Distance (ticks)", Order = 6, GroupName = "7. Chop ochrana")]
		public int VwapChopMaxDistanceTicks { get; set; }

		[NinjaScriptProperty][Range(1, 100)]
		[Display(Name = "VWAP Chop Min Near Bars", Order = 7, GroupName = "7. Chop ochrana")]
		public int VwapChopMinNearBars { get; set; }

		[NinjaScriptProperty][Range(0, 20)]
		[Display(Name = "Max Consecutive Losses/Day", Description = "0 = vypnuto. Po X SL v řadě stop na den.", Order = 8, GroupName = "7. Chop ochrana")]
		public int MaxConsecutiveLossesPerDay { get; set; }

		[NinjaScriptProperty][Range(0, double.MaxValue)]
		[Display(Name = "Daily Loss Stop ($)", Description = "Přísnější stop než Daily Loss Limit. 0 = vypnuto.", Order = 9, GroupName = "7. Chop ochrana")]
		public double DailyLossStopAmount { get; set; }

		[NinjaScriptProperty][Display(Name = "Require VWAP Slope", Order = 10, GroupName = "7. Chop ochrana")]
		public bool RequireVwapSlope { get; set; }

		[NinjaScriptProperty][Range(1, 50)]
		[Display(Name = "VWAP Slope Lookback Bars", Order = 11, GroupName = "7. Chop ochrana")]
