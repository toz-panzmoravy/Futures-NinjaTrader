...
{
	public class MES500TSqueezeMomentumV38 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500T Squeeze Momentum V38 — band reversal NÁKUP/PRODEJ u KC pásma + exit assessment.";
				Name = "MES500TSqueezeMomentumV38";
			...
			bool macdTangle)
		{
			...
			string tag = "MES500TV38_Ring_" + CurrentBar;
		...
		{
			string prefix = "MES500TV38_Hint_" + bar;
		...
		{
			string tag = "MES500TV38_" + tradeId + "_ExitWarn";
		...
		{
			string tag = "MES500TV38_Hint_" + CurrentBar;
		...
		private void ResetTradeTracking()
		{
			...
			postPeakMaxTicks = 0;
			openIsReEntry = false;
			RemoveDrawObject("MES500TV38_Trail");
			RemoveDrawObject("MES500TV38_Char");
			RemoveDrawObject("MES500TV38_Char_Sub");
			RemoveDrawObject("MES500TV38_Char_Hint");
		}
		...
		private void UpdateLiveTradeCharacter(bool isLong, double kcMid, double hist0, double hist1, double hist2)
		{
			...
			Draw.Text(this, "MES500TV38_Char", false, mainLabel, 0, labelY, 0, brush,
		...
		private void DrawActiveTrailLine(bool isLong)
		{
			...
			Draw.Line(this, "MES500TV38_Trail", false, 1, trailStopPrice, 0, trailStopPrice,
		...
		private void UpdateRecoveryWatch()
		{
			...
			{
				recoveryWatchActive = false;
				RemoveDrawObject("MES500TV38_Missed");
				return;
			}
			...
			if (favorable >= RunnerTargetTicks && !recoveryMissShown)
			{
				...
				Draw.TextFixed(this, "MES500TV38_Missed", text, TextPosition.BottomLeft,
		...
		{
			RemoveDrawObject("MES500TV38_Situation");
		...
		private void DrawReleaseMarker(SqueezeReleaseDirection direction)
		{
			...
			string tag = "MES500TV38_Release_" + CurrentBar;
		...
		{
			string tag = "MES500TV38_" + tradeId + "_Long";
		...
		{
			string tag = "MES500TV38_" + tradeId + "_Short";
		...
		private void DrawCloseSignal(int tradeId, bool wasLong, CloseReason reason, int entryBar, double entryPrice, double pnlTicks, CloseHoldVerdict holdVerdict)
		{
			...
			string tag = "MES500TV38_" + tradeId + "_Close";
		...
		private void DrawStatsPanel()
		{
			...
			Draw.TextFixed(this, "MES500TV38_Stats", statsText, TextPosition.TopRight,
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSqueezeMomentumV38[] cacheMES500TSqueezeMomentumV38;
		public MES500TSqueezeMomentumV38 MES500TSqueezeMomentumV38(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return MES500TSqueezeMomentumV38(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public MES500TSqueezeMomentumV38 MES500TSqueezeMomentumV38(ISeries<double> input, bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			if (cacheMES500TSqueezeMomentumV38 != null)
				for (int idx = 0; idx < cacheMES500TSqueezeMomentumV38.Length; idx++)
					if (cacheMES500TSqueezeMomentumV38[idx] != null && cacheMES500TSqueezeMomentumV38[idx].ManualTradeMode == manualTradeMode && cacheMES500TSqueezeMomentumV38[idx].MinBarsBeforeSoftClose == minBarsBeforeSoftClose && cacheMES500TSqueezeMomentumV38[idx].AllowEarlyFlipClose == allowEarlyFlipClose && cacheMES500TSqueezeMomentumV38[idx].ShowHoldWindowTag == showHoldWindowTag && cacheMES500TSqueezeMomentumV38[idx].SuppressExitWarningInHold == suppressExitWarningInHold && cacheMES500TSqueezeMomentumV38[idx].EnableTrendRunnerMode == enableTrendRunnerMode && cacheMES500TSqueezeMomentumV38[idx].TrendRunnerMinPeakTicks == trendRunnerMinPeakTicks && cacheMES500TSqueezeMomentumV38[idx].TrendRunnerKcSlopeBars == trendRunnerKcSlopeBars && cacheMES500TSqueezeMomentumV38[idx].TrendRunnerFadeBars == trendRunnerFadeBars && cacheMES500TSqueezeMomentumV38[idx].TrendRunnerGivebackMult == trendRunnerGivebackMult && cacheMES500TSqueezeMomentumV38[idx].TrendRunnerTrailMult == trendRunnerTrailMult && cacheMES500TSqueezeMomentumV38[idx].EntryMinKcSlopeBars == entryMinKcSlopeBars && cacheMES500TSqueezeMomentumV38[idx].BlockCounterTrendIgnition == blockCounterTrendIgnition && cacheMES500TSqueezeMomentumV38[idx].SuppressExhaustionInRunner == suppressExhaustionInRunner && cacheMES500TSqueezeMomentumV38[idx].OppositeSignalNeedsFade == oppositeSignalNeedsFade && cacheMES500TSqueezeMomentumV38[idx].ShowExitAssessment == showExitAssessment && cacheMES500TSqueezeMomentumV38[idx].EnableBandReversalEntries == enableBandReversalEntries && cacheMES500TSqueezeMomentumV38[idx].BandReversalNearTicks == bandReversalNearTicks && cacheMES500TSqueezeMomentumV38[idx].AllowReversalInSqueeze == allowReversalInSqueeze && cacheMES500TSqueezeMomentumV38[idx].ShowApproachRing == showApproachRing && cacheMES500TSqueezeMomentumV38[idx].ApproachRingMinScore == approachRingMinScore && cacheMES500TSqueezeMomentumV38[idx].ApproachRingSizeTicks == approachRingSizeTicks && cacheMES500TSqueezeMomentumV38[idx].ApproachRingMinOpacity == approachRingMinOpacity && cacheMES500TSqueezeMomentumV38[idx].ApproachRingMaxOpacity == approachRingMaxOpacity && cacheMES500TSqueezeMomentumV38[idx].EnableScreenPopup == enableScreenPopup && cacheMES500TSqueezeMomentumV38[idx].PopupDurationSec == popupDurationSec && cacheMES500TSqueezeMomentumV38[idx].PopupOnApproach == popupOnApproach && cacheMES500TSqueezeMomentumV38[idx].PopupOnEntry == popupOnEntry && cacheMES500TSqueezeMomentumV38[idx].PopupOnClose == popupOnClose && cacheMES500TSqueezeMomentumV38[idx].ShowCloseHoldAdvice == showCloseHoldAdvice && cacheMES500TSqueezeMomentumV38[idx].EnableSignalAutoCorrect == enableSignalAutoCorrect && cacheMES500TSqueezeMomentumV38[idx].SignalReviewBars == signalReviewBars && cacheMES500TSqueezeMomentumV38[idx].SignalMinBarsBeforeReview == signalMinBarsBeforeReview && cacheMES500TSqueezeMomentumV38[idx].FalseSignalAdverseTicks == falseSignalAdverseTicks && cacheMES500TSqueezeMomentumV38[idx].SignalConfirmTicks == signalConfirmTicks && cacheMES500TSqueezeMomentumV38[idx].BlockWeakReEntrySignals == blockWeakReEntrySignals && cacheMES500TSqueezeMomentumV38[idx].ReEntryMinBarsSinceClose == reEntryMinBarsSinceClose && cacheMES500TSqueezeMomentumV38[idx].ReEntryRequireGradeBOrBetter == reEntryRequireGradeBOrBetter && cacheMES500TSqueezeMomentumV38[idx].BlockReEntryAfterChopClose == blockReEntryAfterChopClose && cacheMES500TSqueezeMomentumV38[idx].BlockReEntryAfterLossClose == blockReEntryAfterLossClose && cacheMES500TSqueezeMomentumV38[idx].ReEntryApproachMinScore == reEntryApproachMinScore && cacheMES500TSqueezeMomentumV38[idx].CntMinKcMidSlopeBars == cntMinKcMidSlopeBars && cacheMES500TSqueezeMomentumV38[idx].ShowTrailLine == showTrailLine && cacheMES500TSqueezeMomentumV38[idx].ShowSignalGrade == showSignalGrade && cacheMES500TSqueezeMomentumV38[idx].ShowTradeCharacter == showTradeCharacter && cacheMES500TSqueezeMomentumV38[idx].ChopQuickPeakTicks == chopQuickPeakTicks && cacheMES500TSqueezeMomentumV38[idx].ChopGivebackTicks == chopGivebackTicks && cacheMES500TSqueezeMomentumV38[idx].RunnerTargetTicks == runnerTargetTicks && cacheMES500TSqueezeMomentumV38[idx].RecoveryWindowSec == recoveryWindowSec && cacheMES500TSqueezeMomentumV38[idx].EnableTaskbarFlash == enableTaskbarFlash && cacheMES500TSqueezeMomentumV38[idx].FlashOnEntry == flashOnEntry && cacheMES500TSqueezeMomentumV38[idx].FlashOnClose == flashOnClose && cacheMES500TSqueezeMomentumV38[idx].FlashOnApproach == flashOnApproach && cacheMES500TSqueezeMomentumV38[idx].ShowApproachHints == showApproachHints && cacheMES500TSqueezeMomentumV38[idx].ShowApproachLabels == showApproachLabels && cacheMES500TSqueezeMomentumV38[idx].ApproachMinScore == approachMinScore && cacheMES500TSqueezeMomentumV38[idx].ApproachNearTicks == approachNearTicks && cacheMES500TSqueezeMomentumV38[idx].ApproachOffsetTicks == approachOffsetTicks && cacheMES500TSqueezeMomentumV38[idx].ReEntryLookbackBars == reEntryLookbackBars && cacheMES500TSqueezeMomentumV38[idx].BbPeriod == bbPeriod && cacheMES500TSqueezeMomentumV38[idx].BbStdDev == bbStdDev && cacheMES500TSqueezeMomentumV38[idx].KcPeriod == kcPeriod && cacheMES500TSqueezeMomentumV38[idx].KcMultiplier == kcMultiplier && cacheMES500TSqueezeMomentumV38[idx].MacdFast == macdFast && cacheMES500TSqueezeMomentumV38[idx].MacdSlow == macdSlow && cacheMES500TSqueezeMomentumV38[idx].MacdSignal == macdSignal && cacheMES500TSqueezeMomentumV38[idx].TangleSeparationTicks == tangleSeparationTicks && cacheMES500TSqueezeMomentumV38[idx].TangleSlopeTicks == tangleSlopeTicks && cacheMES500TSqueezeMomentumV38[idx].RequireThreeBarMomentum == requireThreeBarMomentum && cacheMES500TSqueezeMomentumV38[idx].RequireDirectionalRelease == requireDirectionalRelease && cacheMES500TSqueezeMomentumV38[idx].EntryBufferTicks == entryBufferTicks && cacheMES500TSqueezeMomentumV38[idx].AllowKcMidEntry == allowKcMidEntry && cacheMES500TSqueezeMomentumV38[idx].AllowMomentumIgnition == allowMomentumIgnition && cacheMES500TSqueezeMomentumV38[idx].AllowMacdCrossEntry == allowMacdCrossEntry && cacheMES500TSqueezeMomentumV38[idx].AllowBarBreakEntry == allowBarBreakEntry && cacheMES500TSqueezeMomentumV38[idx].AllowTrendContinuationEntry == allowTrendContinuationEntry && cacheMES500TSqueezeMomentumV38[idx].AllowPartialSqueezeEntry == allowPartialSqueezeEntry && cacheMES500TSqueezeMomentumV38[idx].BlockFullSqueezeOnly == blockFullSqueezeOnly && cacheMES500TSqueezeMomentumV38[idx].UseIntrabarEntries == useIntrabarEntries && cacheMES500TSqueezeMomentumV38[idx].RequireBounceForContinuation == requireBounceForContinuation && cacheMES500TSqueezeMomentumV38[idx].RequireKcMidTrend == requireKcMidTrend && cacheMES500TSqueezeMomentumV38[idx].MinHistStepTicks == minHistStepTicks && cacheMES500TSqueezeMomentumV38[idx].EntryCooldownBars == entryCooldownBars && cacheMES500TSqueezeMomentumV38[idx].KcBreakConfirmBars == kcBreakConfirmBars && cacheMES500TSqueezeMomentumV38[idx].MinBarsInTrade == minBarsInTrade && cacheMES500TSqueezeMomentumV38[idx].RequireMomentumFadeForClose == requireMomentumFadeForClose && cacheMES500TSqueezeMomentumV38[idx].UseExhaustionForClose == useExhaustionForClose && cacheMES500TSqueezeMomentumV38[idx].UseAtrTrail == useAtrTrail && cacheMES500TSqueezeMomentumV38[idx].AtrTrailMultiplier == atrTrailMultiplier && cacheMES500TSqueezeMomentumV38[idx].UseIntrabarExits == useIntrabarExits && cacheMES500TSqueezeMomentumV38[idx].ShowExitWarning == showExitWarning && cacheMES500TSqueezeMomentumV38[idx].EnableExitWarningAlerts == enableExitWarningAlerts && cacheMES500TSqueezeMomentumV38[idx].UseProfitGivebackExit == useProfitGivebackExit && cacheMES500TSqueezeMomentumV38[idx].MinProfitForGiveback == minProfitForGiveback && cacheMES500TSqueezeMomentumV38[idx].ProfitGivebackTicks == profitGivebackTicks && cacheMES500TSqueezeMomentumV38[idx].EnableAlerts == enableAlerts && cacheMES500TSqueezeMomentumV38[idx].ShowSignals == showSignals && cacheMES500TSqueezeMomentumV38[idx].ShowSignalLabels == showSignalLabels && cacheMES500TSqueezeMomentumV38[idx].ShowCloseSignals == showCloseSignals && cacheMES500TSqueezeMomentumV38[idx].ShowCloseLabels == showCloseLabels && cacheMES500TSqueezeMomentumV38[idx].ShowCloseReason == showCloseReason && cacheMES500TSqueezeMomentumV38[idx].ShowTradeLines == showTradeLines && cacheMES500TSqueezeMomentumV38[idx].ShowReleaseMarker == showReleaseMarker && cacheMES500TSqueezeMomentumV38[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSqueezeMomentumV38[idx].ShowLiveSituationPanel == showLiveSituationPanel && cacheMES500TSqueezeMomentumV38[idx].ShowKcMid == showKcMid && cacheMES500TSqueezeMomentumV38[idx].EnableCloseAlerts == enableCloseAlerts && cacheMES500TSqueezeMomentumV38[idx].ShowEntryTrigger == showEntryTrigger && cacheMES500TSqueezeMomentumV38[idx].ShowSqueezeBackground == showSqueezeBackground && cacheMES500TSqueezeMomentumV38[idx].ArrowOffsetTicks == arrowOffsetTicks && cacheMES500TSqueezeMomentumV38[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSqueezeMomentumV38[idx].CloseOffsetTicks == closeOffsetTicks && cacheMES500TSqueezeMomentumV38[idx].EqualsInput(input))
						return cacheMES500TSqueezeMomentumV38[idx];
			return CacheIndicator<MES500TSqueezeMomentumV38>(new MES500TSqueezeMomentumV38(){ ManualTradeMode = manualTradeMode, MinBarsBeforeSoftClose = minBarsBeforeSoftClose, AllowEarlyFlipClose = allowEarlyFlipClose, ShowHoldWindowTag = showHoldWindowTag, SuppressExitWarningInHold = suppressExitWarningInHold, EnableTrendRunnerMode = enableTrendRunnerMode, TrendRunnerMinPeakTicks = trendRunnerMinPeakTicks, TrendRunnerKcSlopeBars = trendRunnerKcSlopeBars, TrendRunnerFadeBars = trendRunnerFadeBars, TrendRunnerGivebackMult = trendRunnerGivebackMult, TrendRunnerTrailMult = trendRunnerTrailMult, EntryMinKcSlopeBars = entryMinKcSlopeBars, BlockCounterTrendIgnition = blockCounterTrendIgnition, SuppressExhaustionInRunner = suppressExhaustionInRunner, OppositeSignalNeedsFade = oppositeSignalNeedsFade, ShowExitAssessment = showExitAssessment, EnableBandReversalEntries = enableBandReversalEntries, BandReversalNearTicks = bandReversalNearTicks, AllowReversalInSqueeze = allowReversalInSqueeze, ShowApproachRing = showApproachRing, ApproachRingMinScore = approachRingMinScore, ApproachRingSizeTicks = approachRingSizeTicks, ApproachRingMinOpacity = approachRingMinOpacity, ApproachRingMaxOpacity = approachRingMaxOpacity, EnableScreenPopup = enableScreenPopup, PopupDurationSec = popupDurationSec, PopupOnApproach = popupOnApproach, PopupOnEntry = popupOnEntry, PopupOnClose = popupOnClose, ShowCloseHoldAdvice = showCloseHoldAdvice, EnableSignalAutoCorrect = enableSignalAutoCorrect, SignalReviewBars = signalReviewBars, SignalMinBarsBeforeReview = signalMinBarsBeforeReview, FalseSignalAdverseTicks = falseSignalAdverseTicks, SignalConfirmTicks = signalConfirmTicks, BlockWeakReEntrySignals = blockWeakReEntrySignals, ReEntryMinBarsSinceClose = reEntryMinBarsSinceClose, ReEntryRequireGradeBOrBetter = reEntryRequireGradeBOrBetter, BlockReEntryAfterChopClose = blockReEntryAfterChopClose, BlockReEntryAfterLossClose = blockReEntryAfterLossClose, ReEntryApproachMinScore = reEntryApproachMinScore, CntMinKcMidSlopeBars = cntMinKcMidSlopeBars, ShowTrailLine = showTrailLine, ShowSignalGrade = showSignalGrade, ShowTradeCharacter = showTradeCharacter, ChopQuickPeakTicks = chopQuickPeakTicks, ChopGivebackTicks = chopGivebackTicks, RunnerTargetTicks = runnerTargetTicks, RecoveryWindowSec = recoveryWindowSec, EnableTaskbarFlash = enableTaskbarFlash, FlashOnEntry = flashOnEntry, FlashOnClose = flashOnClose, FlashOnApproach = flashOnApproach, ShowApproachHints = showApproachHints, ShowApproachLabels = showApproachLabels, ApproachMinScore = approachMinScore, ApproachNearTicks = approachNearTicks, ApproachOffsetTicks = approachOffsetTicks, ReEntryLookbackBars = reEntryLookbackBars, BbPeriod = bbPeriod, BbStdDev = bbStdDev, KcPeriod = kcPeriod, KcMultiplier = kcMultiplier, MacdFast = macdFast, MacdSlow = macdSlow, MacdSignal = macdSignal, TangleSeparationTicks = tangleSeparationTicks, TangleSlopeTicks = tangleSlopeTicks, RequireThreeBarMomentum = requireThreeBarMomentum, RequireDirectionalRelease = requireDirectionalRelease, EntryBufferTicks = entryBufferTicks, AllowKcMidEntry = allowKcMidEntry, AllowMomentumIgnition = allowMomentumIgnition, AllowMacdCrossEntry = allowMacdCrossEntry, AllowBarBreakEntry = allowBarBreakEntry, AllowTrendContinuationEntry = allowTrendContinuationEntry, AllowPartialSqueezeEntry = allowPartialSqueezeEntry, BlockFullSqueezeOnly = blockFullSqueezeOnly, UseIntrabarEntries = useIntrabarEntries, RequireBounceForContinuation = requireBounceForContinuation, RequireKcMidTrend = requireKcMidTrend, MinHistStepTicks = minHistStepTicks, EntryCooldownBars = entryCooldownBars, KcBreakConfirmBars = kcBreakConfirmBars, MinBarsInTrade = minBarsInTrade, RequireMomentumFadeForClose = requireMomentumFadeForClose, UseExhaustionForClose = useExhaustionForClose, UseAtrTrail = useAtrTrail, AtrTrailMultiplier = atrTrailMultiplier, UseIntrabarExits = useIntrabarExits, ShowExitWarning = showExitWarning, EnableExitWarningAlerts = enableExitWarningAlerts, UseProfitGivebackExit = useProfitGivebackExit, MinProfitForGiveback = minProfitForGiveback, ProfitGivebackTicks = profitGivebackTicks, EnableAlerts = enableAlerts, ShowSignals = showSignals, ShowSignalLabels = showSignalLabels, ShowCloseSignals = showCloseSignals, ShowCloseLabels = showCloseLabels, ShowCloseReason = showCloseReason, ShowTradeLines = showTradeLines, ShowReleaseMarker = showReleaseMarker, ShowStatsPanel = showStatsPanel, ShowLiveSituationPanel = showLiveSituationPanel, ShowKcMid = showKcMid, EnableCloseAlerts = enableCloseAlerts, ShowEntryTrigger = showEntryTrigger, ShowSqueezeBackground = showSqueezeBackground, ArrowOffsetTicks = arrowOffsetTicks, LabelOffsetTicks = labelOffsetTicks, CloseOffsetTicks = closeOffsetTicks }, input, ref cacheMES500TSqueezeMomentumV38);
		}
	}
}
