...
{
	/// <summary>
	/// MES Microtrend Prop v1.04 – „jeď mikrotrend“: EMA stack = směr, entry = break swingu.
	/// Produkční preset: PriorBarBreak, TICK 200, L+S, SL 40 / PT 56.
	/// </summary>
	public class MesMicrotrendProp_v104 : Strategy
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES Microtrend v1.04 – PriorBarBreak, EMA 9/21 trend ride. TICK 200, L+S, prop risk.";
				Name = "MesMicrotrendProp_v104";
				...
				EntryMode              = TrendRideEntryMode.PriorBarBreak;
				SwingLookbackBars      = 4;
				PriorBarMarginTicks    = 0;
				PullbackTouchTicks     = 3;
				RequireTrendBar        = true;
				MinTrendBodyTicks      = 1;
				MinBarsBetweenEntries  = 2;
				SessionBias            = SessionBiasMode.Off;

				EnableBreakEven       = false;
				BreakEvenTriggerTicks = 35;
				...
				EntryEndTime   = DateTime.Parse("21:00", CultureInfo.InvariantCulture);
		...
		private void WarnTickChart()
		{
			if (tickWarnShown) return;
			if (BarsPeriod.BarsPeriodType != BarsPeriodType.Tick)
				Print(string.Format("{0}: Navržen TICK 200 graf.", Name));
			tickWarnShown = true;
		}
		#endregion
