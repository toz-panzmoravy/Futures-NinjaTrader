...
{
	public class MES500TSqueezeMomentumV39 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "MES500T V39 — jasné vstupy NÁKUP/PRODEJ pro ruční obchod, trend leg + band reversal + exit assessment.";
				Name = "MES500TSqueezeMomentumV39";
				...
				EnableScreenPopup = true;
				PopupDurationSec = 2;
				PopupOnApproach = true;
				PopupOnEntry = true;
				PopupOnClose = true;
				CntMinKcMidSlopeBars = 5;
				ShowTrailLine = true;
				ShowSignalGrade = true;
				ShowTradeCharacter = true;
			...
			bool macdTangle)
		{
			...
			string tag = "MES500TV39_Ring_" + CurrentBar;
		...
		{
			string prefix = "MES500TV39_Hint_" + bar;
		...
		{
			string tag = "MES500TV39_" + tradeId + "_ExitWarn";
		...
		{
			string tag = "MES500TV39_Hint_" + CurrentBar;
		...
		private void ResetTradeTracking()
		{
			...
			postPeakMaxTicks = 0;
			openIsReEntry = false;
			RemoveDrawObject("MES500TV39_Trail");
			RemoveDrawObject("MES500TV39_Char");
			RemoveDrawObject("MES500TV39_Char_Sub");
			RemoveDrawObject("MES500TV39_Char_Hint");
		}
		...
		private void UpdateLiveTradeCharacter(bool isLong, double kcMid, double hist0, double hist1, double hist2)
		{
			...
			Draw.Text(this, "MES500TV39_Char", false, mainLabel, 0, labelY, 0, brush,
		...
		private void DrawActiveTrailLine(bool isLong)
		{
			...
			Draw.Line(this, "MES500TV39_Trail", false, 1, trailStopPrice, 0, trailStopPrice,
		...
		private void UpdateRecoveryWatch()
		{
			...
			{
				recoveryWatchActive = false;
				RemoveDrawObject("MES500TV39_Missed");
				return;
			}
			...
			if (favorable >= RunnerTargetTicks && !recoveryMissShown)
			{
				...
				Draw.TextFixed(this, "MES500TV39_Missed", text, TextPosition.BottomLeft,
		...
		{
			postedSignals.RemoveAll(s => s.Voided || CurrentBar - s.Bar > SignalReviewBars + 8);
		}
		...
		{
			RemoveDrawObject("MES500TV39_Situation");
		...
		private void DrawReleaseMarker(SqueezeReleaseDirection direction)
		{
			...
			string tag = "MES500TV39_Release_" + CurrentBar;
		...
		{
			string tag = "MES500TV39_EntryHint_" + CurrentBar + (isLong ? "_L" : "_S");
		...
		private void DrawCloseSignal(int tradeId, bool wasLong, CloseReason reason, int entryBar, double entryPrice, double pnlTicks, CloseHoldVerdict holdVerdict)
		{
			...
			string tag = "MES500TV39_" + tradeId + "_Close";
		...
		private void DrawStatsPanel()
		{
			...
			Draw.TextFixed(this, "MES500TV39_Stats", statsText, TextPosition.TopRight,
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private MES500TSqueezeMomentumV39[] cacheMES500TSqueezeMomentumV39;
		public MES500TSqueezeMomentumV39 MES500TSqueezeMomentumV39(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return MES500TSqueezeMomentumV39(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public MES500TSqueezeMomentumV39 MES500TSqueezeMomentumV39(ISeries<double> input, bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			if (cacheMES500TSqueezeMomentumV39 != null)
				for (int idx = 0; idx < cacheMES500TSqueezeMomentumV39.Length; idx++)
					if (cacheMES500TSqueezeMomentumV39[idx] != null && cacheMES500TSqueezeMomentumV39[idx].ManualTradeMode == manualTradeMode && cacheMES500TSqueezeMomentumV39[idx].MinBarsBeforeSoftClose == minBarsBeforeSoftClose && cacheMES500TSqueezeMomentumV39[idx].AllowEarlyFlipClose == allowEarlyFlipClose && cacheMES500TSqueezeMomentumV39[idx].ShowHoldWindowTag == showHoldWindowTag && cacheMES500TSqueezeMomentumV39[idx].SuppressExitWarningInHold == suppressExitWarningInHold && cacheMES500TSqueezeMomentumV39[idx].EnableTrendRunnerMode == enableTrendRunnerMode && cacheMES500TSqueezeMomentumV39[idx].TrendRunnerMinPeakTicks == trendRunnerMinPeakTicks && cacheMES500TSqueezeMomentumV39[idx].TrendRunnerKcSlopeBars == trendRunnerKcSlopeBars && cacheMES500TSqueezeMomentumV39[idx].TrendRunnerFadeBars == trendRunnerFadeBars && cacheMES500TSqueezeMomentumV39[idx].TrendRunnerGivebackMult == trendRunnerGivebackMult && cacheMES500TSqueezeMomentumV39[idx].TrendRunnerTrailMult == trendRunnerTrailMult && cacheMES500TSqueezeMomentumV39[idx].EntryMinKcSlopeBars == entryMinKcSlopeBars && cacheMES500TSqueezeMomentumV39[idx].BlockCounterTrendIgnition == blockCounterTrendIgnition && cacheMES500TSqueezeMomentumV39[idx].SuppressExhaustionInRunner == suppressExhaustionInRunner && cacheMES500TSqueezeMomentumV39[idx].OppositeSignalNeedsFade == oppositeSignalNeedsFade && cacheMES500TSqueezeMomentumV39[idx].ShowExitAssessment == showExitAssessment && cacheMES500TSqueezeMomentumV39[idx].EnableBandReversalEntries == enableBandReversalEntries && cacheMES500TSqueezeMomentumV39[idx].BandReversalNearTicks == bandReversalNearTicks && cacheMES500TSqueezeMomentumV39[idx].AllowReversalInSqueeze == allowReversalInSqueeze && cacheMES500TSqueezeMomentumV39[idx].ShowTrendLegEntries == showTrendLegEntries && cacheMES500TSqueezeMomentumV39[idx].RelaxReEntryInTrend == relaxReEntryInTrend && cacheMES500TSqueezeMomentumV39[idx].ShowBlockedEntryHints == showBlockedEntryHints && cacheMES500TSqueezeMomentumV39[idx].ShowApproachRing == showApproachRing && cacheMES500TSqueezeMomentumV39[idx].ApproachRingMinScore == approachRingMinScore && cacheMES500TSqueezeMomentumV39[idx].ApproachRingSizeTicks == approachRingSizeTicks && cacheMES500TSqueezeMomentumV39[idx].ApproachRingMinOpacity == approachRingMinOpacity && cacheMES500TSqueezeMomentumV39[idx].ApproachRingMaxOpacity == approachRingMaxOpacity && cacheMES500TSqueezeMomentumV39[idx].EnableScreenPopup == enableScreenPopup && cacheMES500TSqueezeMomentumV39[idx].PopupDurationSec == popupDurationSec && cacheMES500TSqueezeMomentumV39[idx].PopupOnApproach == popupOnApproach && cacheMES500TSqueezeMomentumV39[idx].PopupOnEntry == popupOnEntry && cacheMES500TSqueezeMomentumV39[idx].PopupOnClose == popupOnClose && cacheMES500TSqueezeMomentumV39[idx].ShowCloseHoldAdvice == showCloseHoldAdvice && cacheMES500TSqueezeMomentumV39[idx].EnableSignalAutoCorrect == enableSignalAutoCorrect && cacheMES500TSqueezeMomentumV39[idx].SignalReviewBars == signalReviewBars && cacheMES500TSqueezeMomentumV39[idx].SignalMinBarsBeforeReview == signalMinBarsBeforeReview && cacheMES500TSqueezeMomentumV39[idx].FalseSignalAdverseTicks == falseSignalAdverseTicks && cacheMES500TSqueezeMomentumV39[idx].SignalConfirmTicks == signalConfirmTicks && cacheMES500TSqueezeMomentumV39[idx].BlockWeakReEntrySignals == blockWeakReEntrySignals && cacheMES500TSqueezeMomentumV39[idx].ReEntryMinBarsSinceClose == reEntryMinBarsSinceClose && cacheMES500TSqueezeMomentumV39[idx].ReEntryRequireGradeBOrBetter == reEntryRequireGradeBOrBetter && cacheMES500TSqueezeMomentumV39[idx].BlockReEntryAfterChopClose == blockReEntryAfterChopClose && cacheMES500TSqueezeMomentumV39[idx].BlockReEntryAfterLossClose == blockReEntryAfterLossClose && cacheMES500TSqueezeMomentumV39[idx].ReEntryApproachMinScore == reEntryApproachMinScore && cacheMES500TSqueezeMomentumV39[idx].CntMinKcMidSlopeBars == cntMinKcMidSlopeBars && cacheMES500TSqueezeMomentumV39[idx].ShowTrailLine == showTrailLine && cacheMES500TSqueezeMomentumV39[idx].ShowSignalGrade == showSignalGrade && cacheMES500TSqueezeMomentumV39[idx].ShowTradeCharacter == showTradeCharacter && cacheMES500TSqueezeMomentumV39[idx].ChopQuickPeakTicks == chopQuickPeakTicks && cacheMES500TSqueezeMomentumV39[idx].ChopGivebackTicks == chopGivebackTicks && cacheMES500TSqueezeMomentumV39[idx].RunnerTargetTicks == runnerTargetTicks && cacheMES500TSqueezeMomentumV39[idx].RecoveryWindowSec == recoveryWindowSec && cacheMES500TSqueezeMomentumV39[idx].EnableTaskbarFlash == enableTaskbarFlash && cacheMES500TSqueezeMomentumV39[idx].FlashOnEntry == flashOnEntry && cacheMES500TSqueezeMomentumV39[idx].FlashOnClose == flashOnClose && cacheMES500TSqueezeMomentumV39[idx].FlashOnApproach == flashOnApproach && cacheMES500TSqueezeMomentumV39[idx].ShowApproachHints == showApproachHints && cacheMES500TSqueezeMomentumV39[idx].ShowApproachLabels == showApproachLabels && cacheMES500TSqueezeMomentumV39[idx].ApproachMinScore == approachMinScore && cacheMES500TSqueezeMomentumV39[idx].ApproachNearTicks == approachNearTicks && cacheMES500TSqueezeMomentumV39[idx].ApproachOffsetTicks == approachOffsetTicks && cacheMES500TSqueezeMomentumV39[idx].ReEntryLookbackBars == reEntryLookbackBars && cacheMES500TSqueezeMomentumV39[idx].BbPeriod == bbPeriod && cacheMES500TSqueezeMomentumV39[idx].BbStdDev == bbStdDev && cacheMES500TSqueezeMomentumV39[idx].KcPeriod == kcPeriod && cacheMES500TSqueezeMomentumV39[idx].KcMultiplier == kcMultiplier && cacheMES500TSqueezeMomentumV39[idx].MacdFast == macdFast && cacheMES500TSqueezeMomentumV39[idx].MacdSlow == macdSlow && cacheMES500TSqueezeMomentumV39[idx].MacdSignal == macdSignal && cacheMES500TSqueezeMomentumV39[idx].TangleSeparationTicks == tangleSeparationTicks && cacheMES500TSqueezeMomentumV39[idx].TangleSlopeTicks == tangleSlopeTicks && cacheMES500TSqueezeMomentumV39[idx].RequireThreeBarMomentum == requireThreeBarMomentum && cacheMES500TSqueezeMomentumV39[idx].RequireDirectionalRelease == requireDirectionalRelease && cacheMES500TSqueezeMomentumV39[idx].EntryBufferTicks == entryBufferTicks && cacheMES500TSqueezeMomentumV39[idx].AllowKcMidEntry == allowKcMidEntry && cacheMES500TSqueezeMomentumV39[idx].AllowMomentumIgnition == allowMomentumIgnition && cacheMES500TSqueezeMomentumV39[idx].AllowMacdCrossEntry == allowMacdCrossEntry && cacheMES500TSqueezeMomentumV39[idx].AllowBarBreakEntry == allowBarBreakEntry && cacheMES500TSqueezeMomentumV39[idx].AllowTrendContinuationEntry == allowTrendContinuationEntry && cacheMES500TSqueezeMomentumV39[idx].AllowPartialSqueezeEntry == allowPartialSqueezeEntry && cacheMES500TSqueezeMomentumV39[idx].BlockFullSqueezeOnly == blockFullSqueezeOnly && cacheMES500TSqueezeMomentumV39[idx].UseIntrabarEntries == useIntrabarEntries && cacheMES500TSqueezeMomentumV39[idx].RequireBounceForContinuation == requireBounceForContinuation && cacheMES500TSqueezeMomentumV39[idx].RequireKcMidTrend == requireKcMidTrend && cacheMES500TSqueezeMomentumV39[idx].MinHistStepTicks == minHistStepTicks && cacheMES500TSqueezeMomentumV39[idx].EntryCooldownBars == entryCooldownBars && cacheMES500TSqueezeMomentumV39[idx].KcBreakConfirmBars == kcBreakConfirmBars && cacheMES500TSqueezeMomentumV39[idx].MinBarsInTrade == minBarsInTrade && cacheMES500TSqueezeMomentumV39[idx].RequireMomentumFadeForClose == requireMomentumFadeForClose && cacheMES500TSqueezeMomentumV39[idx].UseExhaustionForClose == useExhaustionForClose && cacheMES500TSqueezeMomentumV39[idx].UseAtrTrail == useAtrTrail && cacheMES500TSqueezeMomentumV39[idx].AtrTrailMultiplier == atrTrailMultiplier && cacheMES500TSqueezeMomentumV39[idx].UseIntrabarExits == useIntrabarExits && cacheMES500TSqueezeMomentumV39[idx].ShowExitWarning == showExitWarning && cacheMES500TSqueezeMomentumV39[idx].EnableExitWarningAlerts == enableExitWarningAlerts && cacheMES500TSqueezeMomentumV39[idx].UseProfitGivebackExit == useProfitGivebackExit && cacheMES500TSqueezeMomentumV39[idx].MinProfitForGiveback == minProfitForGiveback && cacheMES500TSqueezeMomentumV39[idx].ProfitGivebackTicks == profitGivebackTicks && cacheMES500TSqueezeMomentumV39[idx].EnableAlerts == enableAlerts && cacheMES500TSqueezeMomentumV39[idx].ShowSignals == showSignals && cacheMES500TSqueezeMomentumV39[idx].ShowSignalLabels == showSignalLabels && cacheMES500TSqueezeMomentumV39[idx].ShowCloseSignals == showCloseSignals && cacheMES500TSqueezeMomentumV39[idx].ShowCloseLabels == showCloseLabels && cacheMES500TSqueezeMomentumV39[idx].ShowCloseReason == showCloseReason && cacheMES500TSqueezeMomentumV39[idx].ShowTradeLines == showTradeLines && cacheMES500TSqueezeMomentumV39[idx].ShowReleaseMarker == showReleaseMarker && cacheMES500TSqueezeMomentumV39[idx].ShowStatsPanel == showStatsPanel && cacheMES500TSqueezeMomentumV39[idx].ShowLiveSituationPanel == showLiveSituationPanel && cacheMES500TSqueezeMomentumV39[idx].ShowKcMid == showKcMid && cacheMES500TSqueezeMomentumV39[idx].EnableCloseAlerts == enableCloseAlerts && cacheMES500TSqueezeMomentumV39[idx].ShowEntryTrigger == showEntryTrigger && cacheMES500TSqueezeMomentumV39[idx].ShowSqueezeBackground == showSqueezeBackground && cacheMES500TSqueezeMomentumV39[idx].ArrowOffsetTicks == arrowOffsetTicks && cacheMES500TSqueezeMomentumV39[idx].LabelOffsetTicks == labelOffsetTicks && cacheMES500TSqueezeMomentumV39[idx].CloseOffsetTicks == closeOffsetTicks && cacheMES500TSqueezeMomentumV39[idx].EqualsInput(input))
						return cacheMES500TSqueezeMomentumV39[idx];
			return CacheIndicator<MES500TSqueezeMomentumV39>(new MES500TSqueezeMomentumV39(){ ManualTradeMode = manualTradeMode, MinBarsBeforeSoftClose = minBarsBeforeSoftClose, AllowEarlyFlipClose = allowEarlyFlipClose, ShowHoldWindowTag = showHoldWindowTag, SuppressExitWarningInHold = suppressExitWarningInHold, EnableTrendRunnerMode = enableTrendRunnerMode, TrendRunnerMinPeakTicks = trendRunnerMinPeakTicks, TrendRunnerKcSlopeBars = trendRunnerKcSlopeBars, TrendRunnerFadeBars = trendRunnerFadeBars, TrendRunnerGivebackMult = trendRunnerGivebackMult, TrendRunnerTrailMult = trendRunnerTrailMult, EntryMinKcSlopeBars = entryMinKcSlopeBars, BlockCounterTrendIgnition = blockCounterTrendIgnition, SuppressExhaustionInRunner = suppressExhaustionInRunner, OppositeSignalNeedsFade = oppositeSignalNeedsFade, ShowExitAssessment = showExitAssessment, EnableBandReversalEntries = enableBandReversalEntries, BandReversalNearTicks = bandReversalNearTicks, AllowReversalInSqueeze = allowReversalInSqueeze, ShowTrendLegEntries = showTrendLegEntries, RelaxReEntryInTrend = relaxReEntryInTrend, ShowBlockedEntryHints = showBlockedEntryHints, ShowApproachRing = showApproachRing, ApproachRingMinScore = approachRingMinScore, ApproachRingSizeTicks = approachRingSizeTicks, ApproachRingMinOpacity = approachRingMinOpacity, ApproachRingMaxOpacity = approachRingMaxOpacity, EnableScreenPopup = enableScreenPopup, PopupDurationSec = popupDurationSec, PopupOnApproach = popupOnApproach, PopupOnEntry = popupOnEntry, PopupOnClose = popupOnClose, ShowCloseHoldAdvice = showCloseHoldAdvice, EnableSignalAutoCorrect = enableSignalAutoCorrect, SignalReviewBars = signalReviewBars, SignalMinBarsBeforeReview = signalMinBarsBeforeReview, FalseSignalAdverseTicks = falseSignalAdverseTicks, SignalConfirmTicks = signalConfirmTicks, BlockWeakReEntrySignals = blockWeakReEntrySignals, ReEntryMinBarsSinceClose = reEntryMinBarsSinceClose, ReEntryRequireGradeBOrBetter = reEntryRequireGradeBOrBetter, BlockReEntryAfterChopClose = blockReEntryAfterChopClose, BlockReEntryAfterLossClose = blockReEntryAfterLossClose, ReEntryApproachMinScore = reEntryApproachMinScore, CntMinKcMidSlopeBars = cntMinKcMidSlopeBars, ShowTrailLine = showTrailLine, ShowSignalGrade = showSignalGrade, ShowTradeCharacter = showTradeCharacter, ChopQuickPeakTicks = chopQuickPeakTicks, ChopGivebackTicks = chopGivebackTicks, RunnerTargetTicks = runnerTargetTicks, RecoveryWindowSec = recoveryWindowSec, EnableTaskbarFlash = enableTaskbarFlash, FlashOnEntry = flashOnEntry, FlashOnClose = flashOnClose, FlashOnApproach = flashOnApproach, ShowApproachHints = showApproachHints, ShowApproachLabels = showApproachLabels, ApproachMinScore = approachMinScore, ApproachNearTicks = approachNearTicks, ApproachOffsetTicks = approachOffsetTicks, ReEntryLookbackBars = reEntryLookbackBars, BbPeriod = bbPeriod, BbStdDev = bbStdDev, KcPeriod = kcPeriod, KcMultiplier = kcMultiplier, MacdFast = macdFast, MacdSlow = macdSlow, MacdSignal = macdSignal, TangleSeparationTicks = tangleSeparationTicks, TangleSlopeTicks = tangleSlopeTicks, RequireThreeBarMomentum = requireThreeBarMomentum, RequireDirectionalRelease = requireDirectionalRelease, EntryBufferTicks = entryBufferTicks, AllowKcMidEntry = allowKcMidEntry, AllowMomentumIgnition = allowMomentumIgnition, AllowMacdCrossEntry = allowMacdCrossEntry, AllowBarBreakEntry = allowBarBreakEntry, AllowTrendContinuationEntry = allowTrendContinuationEntry, AllowPartialSqueezeEntry = allowPartialSqueezeEntry, BlockFullSqueezeOnly = blockFullSqueezeOnly, UseIntrabarEntries = useIntrabarEntries, RequireBounceForContinuation = requireBounceForContinuation, RequireKcMidTrend = requireKcMidTrend, MinHistStepTicks = minHistStepTicks, EntryCooldownBars = entryCooldownBars, KcBreakConfirmBars = kcBreakConfirmBars, MinBarsInTrade = minBarsInTrade, RequireMomentumFadeForClose = requireMomentumFadeForClose, UseExhaustionForClose = useExhaustionForClose, UseAtrTrail = useAtrTrail, AtrTrailMultiplier = atrTrailMultiplier, UseIntrabarExits = useIntrabarExits, ShowExitWarning = showExitWarning, EnableExitWarningAlerts = enableExitWarningAlerts, UseProfitGivebackExit = useProfitGivebackExit, MinProfitForGiveback = minProfitForGiveback, ProfitGivebackTicks = profitGivebackTicks, EnableAlerts = enableAlerts, ShowSignals = showSignals, ShowSignalLabels = showSignalLabels, ShowCloseSignals = showCloseSignals, ShowCloseLabels = showCloseLabels, ShowCloseReason = showCloseReason, ShowTradeLines = showTradeLines, ShowReleaseMarker = showReleaseMarker, ShowStatsPanel = showStatsPanel, ShowLiveSituationPanel = showLiveSituationPanel, ShowKcMid = showKcMid, EnableCloseAlerts = enableCloseAlerts, ShowEntryTrigger = showEntryTrigger, ShowSqueezeBackground = showSqueezeBackground, ArrowOffsetTicks = arrowOffsetTicks, LabelOffsetTicks = labelOffsetTicks, CloseOffsetTicks = closeOffsetTicks }, input, ref cacheMES500TSqueezeMomentumV39);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.MES500TSqueezeMomentumV39 MES500TSqueezeMomentumV39(bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV39(Input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}

		public Indicators.MES500TSqueezeMomentumV39 MES500TSqueezeMomentumV39(ISeries<double> input , bool manualTradeMode, int minBarsBeforeSoftClose, bool allowEarlyFlipClose, bool showHoldWindowTag, bool suppressExitWarningInHold, bool enableTrendRunnerMode, int trendRunnerMinPeakTicks, int trendRunnerKcSlopeBars, int trendRunnerFadeBars, double trendRunnerGivebackMult, double trendRunnerTrailMult, int entryMinKcSlopeBars, bool blockCounterTrendIgnition, bool suppressExhaustionInRunner, bool oppositeSignalNeedsFade, bool showExitAssessment, bool enableBandReversalEntries, int bandReversalNearTicks, bool allowReversalInSqueeze, bool showTrendLegEntries, bool relaxReEntryInTrend, bool showBlockedEntryHints, bool showApproachRing, int approachRingMinScore, int approachRingSizeTicks, double approachRingMinOpacity, double approachRingMaxOpacity, bool enableScreenPopup, int popupDurationSec, bool popupOnApproach, bool popupOnEntry, bool popupOnClose, bool showCloseHoldAdvice, bool enableSignalAutoCorrect, int signalReviewBars, int signalMinBarsBeforeReview, int falseSignalAdverseTicks, int signalConfirmTicks, bool blockWeakReEntrySignals, int reEntryMinBarsSinceClose, bool reEntryRequireGradeBOrBetter, bool blockReEntryAfterChopClose, bool blockReEntryAfterLossClose, int reEntryApproachMinScore, int cntMinKcMidSlopeBars, bool showTrailLine, bool showSignalGrade, bool showTradeCharacter, int chopQuickPeakTicks, int chopGivebackTicks, int runnerTargetTicks, int recoveryWindowSec, bool enableTaskbarFlash, bool flashOnEntry, bool flashOnClose, bool flashOnApproach, bool showApproachHints, bool showApproachLabels, int approachMinScore, int approachNearTicks, int approachOffsetTicks, int reEntryLookbackBars, int bbPeriod, double bbStdDev, int kcPeriod, double kcMultiplier, int macdFast, int macdSlow, int macdSignal, int tangleSeparationTicks, int tangleSlopeTicks, bool requireThreeBarMomentum, bool requireDirectionalRelease, int entryBufferTicks, bool allowKcMidEntry, bool allowMomentumIgnition, bool allowMacdCrossEntry, bool allowBarBreakEntry, bool allowTrendContinuationEntry, bool allowPartialSqueezeEntry, bool blockFullSqueezeOnly, bool useIntrabarEntries, bool requireBounceForContinuation, bool requireKcMidTrend, int minHistStepTicks, int entryCooldownBars, int kcBreakConfirmBars, int minBarsInTrade, bool requireMomentumFadeForClose, bool useExhaustionForClose, bool useAtrTrail, double atrTrailMultiplier, bool useIntrabarExits, bool showExitWarning, bool enableExitWarningAlerts, bool useProfitGivebackExit, int minProfitForGiveback, int profitGivebackTicks, bool enableAlerts, bool showSignals, bool showSignalLabels, bool showCloseSignals, bool showCloseLabels, bool showCloseReason, bool showTradeLines, bool showReleaseMarker, bool showStatsPanel, bool showLiveSituationPanel, bool showKcMid, bool enableCloseAlerts, bool showEntryTrigger, bool showSqueezeBackground, int arrowOffsetTicks, int labelOffsetTicks, int closeOffsetTicks)
		{
			return indicator.MES500TSqueezeMomentumV39(input, manualTradeMode, minBarsBeforeSoftClose, allowEarlyFlipClose, showHoldWindowTag, suppressExitWarningInHold, enableTrendRunnerMode, trendRunnerMinPeakTicks, trendRunnerKcSlopeBars, trendRunnerFadeBars, trendRunnerGivebackMult, trendRunnerTrailMult, entryMinKcSlopeBars, blockCounterTrendIgnition, suppressExhaustionInRunner, oppositeSignalNeedsFade, showExitAssessment, enableBandReversalEntries, bandReversalNearTicks, allowReversalInSqueeze, showTrendLegEntries, relaxReEntryInTrend, showBlockedEntryHints, showApproachRing, approachRingMinScore, approachRingSizeTicks, approachRingMinOpacity, approachRingMaxOpacity, enableScreenPopup, popupDurationSec, popupOnApproach, popupOnEntry, popupOnClose, showCloseHoldAdvice, enableSignalAutoCorrect, signalReviewBars, signalMinBarsBeforeReview, falseSignalAdverseTicks, signalConfirmTicks, blockWeakReEntrySignals, reEntryMinBarsSinceClose, reEntryRequireGradeBOrBetter, blockReEntryAfterChopClose, blockReEntryAfterLossClose, reEntryApproachMinScore, cntMinKcMidSlopeBars, showTrailLine, showSignalGrade, showTradeCharacter, chopQuickPeakTicks, chopGivebackTicks, runnerTargetTicks, recoveryWindowSec, enableTaskbarFlash, flashOnEntry, flashOnClose, flashOnApproach, showApproachHints, showApproachLabels, approachMinScore, approachNearTicks, approachOffsetTicks, reEntryLookbackBars, bbPeriod, bbStdDev, kcPeriod, kcMultiplier, macdFast, macdSlow, macdSignal, tangleSeparationTicks, tangleSlopeTicks, requireThreeBarMomentum, requireDirectionalRelease, entryBufferTicks, allowKcMidEntry, allowMomentumIgnition, allowMacdCrossEntry, allowBarBreakEntry, allowTrendContinuationEntry, allowPartialSqueezeEntry, blockFullSqueezeOnly, useIntrabarEntries, requireBounceForContinuation, requireKcMidTrend, minHistStepTicks, entryCooldownBars, kcBreakConfirmBars, minBarsInTrade, requireMomentumFadeForClose, useExhaustionForClose, useAtrTrail, atrTrailMultiplier, useIntrabarExits, showExitWarning, enableExitWarningAlerts, useProfitGivebackExit, minProfitForGiveback, profitGivebackTicks, enableAlerts, showSignals, showSignalLabels, showCloseSignals, showCloseLabels, showCloseReason, showTradeLines, showReleaseMarker, showStatsPanel, showLiveSituationPanel, showKcMid, enableCloseAlerts, showEntryTrigger, showSqueezeBackground, arrowOffsetTicks, labelOffsetTicks, closeOffsetTicks);
		}
	}
